...
/**
 * Implementation of {@link org.luaj.vm2.Globals.Compiler} which does direct lua-to-java-bytecode
 * compiling.
 *
 * <p>By default, when using {@link org.luaj.vm2.lib.jse.JsePlatform} to construct globals, the
 * plain compiler {@link LuaC} is installed and lua code will only be compiled into lua bytecode and
 * execute as {@link LuaClosure}.
 *
 * <p>To override the default compiling behavior with {@link LuaJC} lua-to-java bytecode compiler,
 * install it before undumping code, for example:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>This requires the bcel library to be on the class path to work as expected. If the library is
 * not found, the default {@link LuaC} lua-to-lua-bytecode compiler will be used.
 *
...
public class LuaJC implements Globals.Loader {
  ...
  /**
   * Install the compiler as the main Globals.Loader to use in a set of globals. Will fall back to
   * the LuaC prototype compiler.
   */
  ...
  public Hashtable compileAll(
      InputStream script, String chunkname, String filename, Globals globals, boolean genmain)
      throws IOException {
  ...
  public Hashtable compileAll(
      Reader script, String chunkname, String filename, Globals globals, boolean genmain)
      throws IOException {
    final String classname = toStandardJavaClassName(chunkname);
  ...
  private Hashtable compileProtoAndSubProtos(
      Prototype p, String classname, String filename, boolean genmain) throws IOException {
