...
public class ProtoInfo {
        ...
        for (int j = 0; j < prototype.maxstacksize; j++) {
          ...
          String u =
              (v == null
                  ? ""
                  : v.upvalue != null
                      ? !v.upvalue.rw ? "[C] " : (v.allocupvalue && v.pc == pc ? "[*] " : "[]  ")
                      : "    ");
  ...
  private VarInfo[][] findVariables() {
    ...
    for (int i = 0; i < v.length; i++) v[i] = new VarInfo[n];
      ...
      for (int slot = 0; slot < m; slot++) {
        ...
        if (nprev == 0) var = params[slot];
          ...
          for (int i = 0; i < nprev; i++) {
            ...
            if (v[slot][bp.pc1] == VarInfo.INVALID) var = VarInfo.INVALID;
          }
        }
        if (var == null) var = VarInfo.PHI(this, slot, b0.pc0);
        v[slot][b0.pc0] = var;
      }
      ...
      for (int pc = b0.pc0; pc <= b0.pc1; pc++) {
        ...
        switch (op) {
          ...
          case Lua.OP_CONCAT: /*	A B C	R(A) := R(B).. ... ..R(C)			*/
            ...
            b = Lua.GETARG_B(ins);
            c = Lua.GETARG_C(ins);
            for (; b <= c; b++) v[b][pc].isreferenced = true;
            v[a][pc] = new VarInfo(a, pc);
            break;
          ...
          case Lua.OP_LOADNIL: /*	A B	R(A) := ... := R(A+B) := nil			*/
            a = Lua.GETARG_A(ins);
            b = Lua.GETARG_B(ins);
            for (; b-- >= 0; a++) v[a][pc] = new VarInfo(a, pc);
            break;

          case Lua.OP_VARARG: /*	A B	R(A), R(A+1), ..., R(A+B-1) = vararg		*/
            a = Lua.GETARG_A(ins);
            b = Lua.GETARG_B(ins);
            for (int j = 1; j < b; j++, a++) v[a][pc] = new VarInfo(a, pc);
            if (b == 0) for (; a < m; a++) v[a][pc] = VarInfo.INVALID;
            break;
          ...
          case Lua.OP_CALL: /*	A B C	R(A), ... ,R(A+C-2) := R(A)(R(A+1), ... ,R(A+B-1)) */
            ...
            v[a][pc].isreferenced = true;
            v[a][pc].isreferenced = true;
            for (int i = 1; i <= b - 1; i++) v[a + i][pc].isreferenced = true;
            for (int j = 0; j <= c - 2; j++, a++) v[a][pc] = new VarInfo(a, pc);
            for (; a < m; a++) v[a][pc] = VarInfo.INVALID;
            break;
          ...
          case Lua.OP_TFORCALL: /* A C	R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));	*/
            ...
            v[a++][pc].isreferenced = true;
            v[a++][pc].isreferenced = true;
            for (int j = 0; j < c; j++, a++) v[a][pc] = new VarInfo(a, pc);
            for (; a < m; a++) v[a][pc] = VarInfo.INVALID;
            break;
          ...
          case Lua.OP_RETURN: /*	A B	return R(A), ... ,R(A+B-2)	(see note)	*/
            a = Lua.GETARG_A(ins);
            b = Lua.GETARG_B(ins);
            for (int i = 0; i <= b - 2; i++) v[a + i][pc].isreferenced = true;
            break;
          ...
          case Lua.OP_CLOSURE:
            {
              ...
              for (int k = 0, nups = upvalues.length; k < nups; ++k)
                if (upvalues[k].instack) v[upvalues[k].idx][pc].isreferenced = true;
              v[a][pc] = new VarInfo(a, pc);
              break;
            }
          ...
          case Lua.OP_SETLIST: /*	A B C	R(A)[(C-1)*FPF+i]:= R(A+i), 1 <= i <= B	*/
            ...
            b = Lua.GETARG_B(ins);
            v[a][pc].isreferenced = true;
            for (int i = 1; i <= b; i++) v[a + i][pc].isreferenced = true;
            break;
          ...
          case Lua.OP_JMP: /*	sBx	pc+=sBx					*/
            a = Lua.GETARG_A(ins);
            if (a > 0) for (--a; a < m; a++) v[a][pc] = VarInfo.INVALID;
            break;
  ...
  private static void propogateVars(VarInfo[][] v, int pcfrom, int pcto) {
    for (int j = 0, m = v.length; j < m; j++) v[j][pcto] = v[j][pcfrom];
  }
  ...
  private void replaceAll(VarInfo[] v, int n, VarInfo vold, VarInfo vnew) {
    for (int i = 0; i < n; i++) if (v[i] == vold) v[i] = vnew;
  }
  ...
  private UpvalInfo findOpenUp(int pc, int slot) {
    if (openups[slot] == null) openups[slot] = new UpvalInfo[prototype.code.length];
  ...
  private String[] findInnerprotoNames() {
    if (prototype.p.length <= 0) return null;
          ...
          case Lua.OP_SETTABUP:
            {
              ...
              if (Lua.ISK(b)) name = prototype.k[b & 0x0ff].tojstring();
              break;
            }
