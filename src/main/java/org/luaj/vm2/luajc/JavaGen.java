...
public class JavaGen {
      ...
      for (int pc = b0.pc0; pc <= b0.pc1; pc++) {
        ...
        switch (o) {
          ...
          case Lua.OP_CONCAT: /*	A B C	R(A):= R(B).. ... ..R(C)			*/
            for (int k = b; k <= c; k++) builder.loadLocal(pc, k);
            if (c > b + 1) {
              builder.tobuffer();
              for (int k = c; --k >= b; ) builder.concatbuffer();
              builder.tovalue();
            } else {
              builder.concatvalue();
            }
          ...
          case Lua.OP_LOADBOOL: /*	A B C	R(A):= (Bool)B: if (C) pc++			*/
            ...
            builder.storeLocal(pc, a);
            if (c != 0) builder.addBranch(pc, JavaBuilder.BRANCH_GOTO, pc + 2);
            break;
          ...
          case Lua.OP_LE: /*	A B C	if ((RK(B) <= RK(C)) ~= A) then pc++  		*/
            ...
            builder.compareop(o);
            builder.addBranch(
                pc, (a != 0 ? JavaBuilder.BRANCH_IFEQ : JavaBuilder.BRANCH_IFNE), pc + 2);
            break;
          ...
          case Lua.OP_TEST: /*	A C	if not (R(A) <=> C) then pc++			*/
            ...
            builder.toBoolean();
            builder.addBranch(
                pc, (c != 0 ? JavaBuilder.BRANCH_IFEQ : JavaBuilder.BRANCH_IFNE), pc + 2);
            break;
          ...
          case Lua.OP_CALL:
            {
              ...
              switch (narg) {
                ...
                case 2:
                case 3:
                  for (int i = 1; i < b; i++) builder.loadLocal(pc, a + i);
                  break;
          ...
          case Lua.OP_CLOSURE: /*	A Bx	R(A):= closure(KPROTO[Bx], R(A), ... ,R(A+n))	*/
            {
              ...
              for (int up = 0; up < nup; ++up) {
                ...
                if (u.instack) builder.closureInitUpvalueFromLocal(protoname, up, pc, u.idx);
  ...
  private void loadLocalOrConstant(Prototype p, JavaBuilder builder, int pc, int borc) {
    ...
    else builder.loadConstant(p.k[borc & 0xff]);
  }
}
