 ...
 *
 * <p>Because lua string values are more nearly sequences of bytes than sequences of characters or
 * unicode code points, the {@link LuaString} implementation holds the string value in an internal
 * byte array.
 *
 * <p>{@link LuaString} values are not considered mutable once constructed, so multiple {@link
 * LuaString} values can chare a single byte array.
 *
 * <p>Currently {@link LuaString}s are pooled via a centrally managed weak table. To ensure that as
 * many string values as possible take advantage of this, Constructors are not exposed directly. As
 * with number, booleans, and nil, instance construction should be via {@link
 * LuaValue#valueOf(byte[])} or similar API.
 *
 * <p>Because of this pooling, users of LuaString <em>must not directly alter the bytes in a
 * LuaString</em>, or undefined behavior will result.
 *
 * <p>When Java Strings are used to initialize {@link LuaString} data, the UTF8 encoding is assumed.
 * The functions {@link #lengthAsUtf8(char[])}, {@link #encodeToUtf8(char[], int, byte[], int)}, and
 * {@link #decodeAsUtf8(byte[], int, int)} are used to convert back and forth between UTF8 byte
...
public class LuaString extends LuaValue {
   ...
   * this is set to the string metatable as a side effect of loading the string library, and is
   * read-write to provide flexible behavior by default. When used in a server environment where
   * there may be roge scripts, this should be replaced with a read-only table since it is shared
   * across all lua code in this Java VM.
   */
  public static LuaValue s_metatable;

  /**
   * The bytes for the string. These <em><b>must not be mutated directly</b></em> because the
   * backing may be shared by multiple LuaStrings, and the hash code is computed only at
   * construction time. It is exposed only for performance and legacy reasons.
   */
  ...
  /**
   * Size of cache of recent short strings. This is the maximum number of LuaStrings that will be
   * retained in the cache of recent short strings. Exposed to package for testing.
   */
  static final int RECENT_STRINGS_CACHE_SIZE = 128;

  /**
   * Maximum length of a string to be considered for recent short strings caching. This effectively
   * limits the total memory that can be spent on the recent strings cache, because no LuaString
   * whose backing exceeds this length will be put into the cache. Exposed to package for testing.
   */
  static final int RECENT_STRINGS_MAX_LENGTH = 32;

  /**
   * Simple cache of recently created strings that are short. This is simply a list of strings,
   * indexed by their hash codes modulo the cache size that have been recently constructed. If a
   * string is being constructed frequently from different contexts, it will generally show up as a
   * cache hit and resolve to the same value.
   */
  ...
  /**
   * Get a {@link LuaString} instance whose bytes match the supplied Java String using the UTF8
   * encoding.
   *
   ...
   *
   * <p>The array is first be used as the backing for this object, so clients must not change
   * contents. If the supplied value for 'len' is more than half the length of the container, the
   * supplied byte array will be used as the backing, otherwise the bytes will be copied to a new
   * byte array, and cache lookup may be performed.
   *
   * <p>
   *
  ...
  public static LuaString valueOf(byte[] bytes, int off, int len) {
    if (len > RECENT_STRINGS_MAX_LENGTH) return valueFromCopy(bytes, off, len);
  ...
  /**
   * Construct a {@link LuaString} around, possibly using the the supplied byte array as the backing
   * store.
   *
   * <p>The caller must ensure that the array is not mutated after the call. However, if the string
   * is short enough the short-string cache is checked for a match which may be used instead of the
   * supplied byte array.
   *
   * <p>
   *
  ...
  public static LuaString valueUsing(byte[] bytes, int off, int len) {
    if (bytes.length > RECENT_STRINGS_MAX_LENGTH) return new LuaString(bytes, off, len);
    final int hash = hashCode(bytes, off, len);
    final int bucket = hash & (RECENT_STRINGS_CACHE_SIZE - 1);
    final LuaString t = RecentShortStrings.recent_short_strings[bucket];
    if (t != null && t.m_hashcode == hash && t.byteseq(bytes, off, len)) return t;
   ...
   *
   * <p>Only the low-order 8-bits of each character are used, the remainder is ignored.
   *
   * <p>This is most useful for constructing byte sequences that do not conform to UTF8.
   *
   * @param bytes array of char, whose values are truncated at 8-bits each and put into a byte
   *     array.
  ...
  public static LuaString valueOf(char[] bytes, int off, int len) {
    byte[] b = new byte[len];
    for (int i = 0; i < len; i++) b[i] = (byte) bytes[i + off];
   ...
   *
   * <p>The LuaString returned will either be a new LuaString containing a copy of the bytes array,
  ...
  /**
   * Construct a {@link LuaString} for all the bytes in a byte array, possibly using the supplied
   * array as the backing store.
   *
   * <p>The LuaString returned will either be a new LuaString containing the byte array, or be an
   * existing LuaString used already having the same value.
   *
   * <p>The caller must not mutate the contents of the byte array after this call, as it may be used
   * elsewhere due to recent short string caching.
   *
   * @param bytes byte buffer
   * @return {@link LuaString} wrapping the byte buffer
   */
   ...
   *
   * <p>The array is used directly after this is called, so clients must not change contents.
   *
   * <p>
   *
   * @param bytes byte buffer
   ...
   * @return LuaString which is a substring whose first character is at offset beginIndex and
   *     extending for (endIndex - beginIndex ) characters.
   */
  ...
  /**
   * Compute the hash code of a sequence of bytes within a byte array using lua's rules for string
   * hashes. For long strings, not all bytes are hashed.
   *
  ...
  public static int lengthAsUtf8(char[] chars) {
    int i, b;
    char c;
    for (i = b = chars.length; --i >= 0; ) if ((c = chars[i]) >= 0x80) b += (c >= 0x800) ? 2 : 1;
    return b;
  }
   ...
   *
   * <p>The string should be measured first with lengthAsUtf8 to make sure the given byte array is
   * large enough.
   *
   * @param chars Array of unicode characters to be encoded as UTF-8
  ...
  /**
   * convert to a number using baee 10 or base 16 if it starts with '0x', or NIL if it can't be
   * converted
   *
  ...
  /**
   * Convert to a number in base 10, or base 16 if the string starts with '0x', or return Double.NaN
  ...
  private double scanlong(int base, int start, int end) {
    ...
    for (int i = (neg ? start + 1 : start); i < end; i++) {
      int digit =
          m_bytes[i]
              - (base <= 10 || (m_bytes[i] >= '0' && m_bytes[i] <= '9')
                  ? '0'
  ...
  private double scandouble(int start, int end) {
    ...
    for (int i = start; i < end; i++) c[i - start] = (char) m_bytes[i];
    try {
  ...
  /**
   * Print the bytes of the LuaString to a PrintStream as if it were an ASCII string, quoting and
