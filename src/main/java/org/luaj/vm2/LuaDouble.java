 ...
 *
 * <p>These instance are not instantiated directly by clients, but indirectly via the static
 * functions {@link LuaValue#valueOf(int)} or {@link LuaValue#valueOf(double)} functions. This
 * ensures that values which can be represented as int are wrapped in {@link LuaInteger} instead of
 * {@link LuaDouble}.
 *
 * <p>Almost all API's implemented in LuaDouble are defined and documented in {@link LuaValue}.
 *
 * <p>However the constants {@link #NAN}, {@link #POSINF}, {@link #NEGINF}, {@link #JSTR_NAN},
 * {@link #JSTR_POSINF}, and {@link #JSTR_NEGINF} may be useful when dealing with Nan or Infinite
 * values.
 *
 * <p>LuaDouble also defines functions for handling the unique math rules of lua devision and modulo
 * in
 *
 * <ul>
 *   <li>{@link #ddiv(double, double)}
 *   <li>{@link #ddiv_d(double, double)}
 *   <li>{@link #dmod(double, double)}
 *   <li>{@link #dmod_d(double, double)}
 * </ul>
 *
 * <p>
 *
...
public class LuaDouble extends LuaNumber {
   ...
   * @return {@link LuaValue} for the result of the division, taking into account positive and
   *     negiative infinity, and Nan
  ...
  public static double ddiv_d(double lhs, double rhs) {
    return rhs != 0
        ? lhs / rhs
        : lhs > 0 ? Double.POSITIVE_INFINITY : lhs == 0 ? Double.NaN : Double.NEGATIVE_INFINITY;
  }
