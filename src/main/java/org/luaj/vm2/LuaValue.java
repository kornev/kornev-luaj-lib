 ...
 *
 * <p>Establishes base implementations for all the operations on lua types. This allows Java clients
 * to deal essentially with one type for all Java values, namely {@link LuaValue}.
 *
 * <p>Constructors are provided as static methods for common Java types, such as {@link
 * LuaValue#valueOf(int)} or {@link LuaValue#valueOf(String)} to allow for instance pooling.
 *
 * <p>Constants are defined for the lua values {@link #NIL}, {@link #TRUE}, and {@link #FALSE}. A
 * constant {@link #NONE} is defined which is a {@link Varargs} list having no values.
 *
 * <p>Operations are performed on values directly via their Java methods. For example, the following
 ...
 * }</pre>
 *
 * Note that in this example, c will be a {@link LuaDouble}, but would be a {@link LuaInteger} if
 * the value of a were changed to 8, say. In general the value of c in practice will vary depending
 * on both the types and values of a and b as well as any metatable/metatag processing that occurs.
 *
 * <p>Field access and function calls are similar, with common overloads to simplify Java usage:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>To supply variable arguments or get multiple return values, use {@link #invoke(Varargs)} or
 * {@link #invokemethod(LuaValue, Varargs)} methods:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>To load and run a script, {@link LoadState} is used:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * For this to work the file must be in the current directory, or in the class path, dependening on
 * the platform. See {@link org.luaj.vm2.lib.jse.JsePlatform} for details.
 *
 * <p>In general a {@link LuaError} may be thrown on any operation when the types supplied to any
 * operation are illegal from a lua perspective. Examples could be attempting to concatenate a NIL
 * value, or attempting arithmetic on values that are not number.
 *
 * <p>There are several methods for preinitializing tables, such as:
 *
 * <ul>
 *   <li>{@link #listOf(LuaValue[])} for unnamed elements
 *   <li>{@link #tableOf(LuaValue[])} for named elements
 *   <li>{@link #tableOf(LuaValue[], LuaValue[], Varargs)} for mixtures
 * </ul>
 *
 * <p>Predefined constants exist for the standard lua type constants {@link #TNIL}, {@link
 * #TBOOLEAN}, {@link #TLIGHTUSERDATA}, {@link #TNUMBER}, {@link #TSTRING}, {@link #TTABLE}, {@link
 * #TFUNCTION}, {@link #TUSERDATA}, {@link #TTHREAD}, and extended lua type constants {@link #TINT},
 * {@link #TNONE}, {@link #TVALUE}
 *
 * <p>Predefined constants exist for all strings used as metatags: {@link #INDEX}, {@link
 * #NEWINDEX}, {@link #CALL}, {@link #MODE}, {@link #METATABLE}, {@link #ADD}, {@link #SUB}, {@link
 * #DIV}, {@link #MUL}, {@link #POW}, {@link #MOD}, {@link #UNM}, {@link #LEN}, {@link #EQ}, {@link
 * #LT}, {@link #LE}, {@link #TOSTRING}, and {@link #CONCAT}.
 *
...
public abstract class LuaValue extends Varargs {
  ...
  /**
   * Type enumeration constant for lua numbers that are ints, for compatibility with lua 5.1 number
   * patch only
   */
  ...
  /**
   * Array of {@link #NIL} values to optimize filling stacks using System.arraycopy(). Must not be
   * modified.
   */
  public static final LuaValue[] NILS = new LuaValue[MAXSTACK];

  static {
    for (int i = 0; i < MAXSTACK; i++) NILS[i] = NIL;
  }
   ...
   *
   * @return value for this type, one of {@link #TNIL}, {@link #TBOOLEAN}, {@link #TNUMBER}, {@link
   *     #TSTRING}, {@link #TTABLE}, {@link #TFUNCTION}, {@link #TUSERDATA}, {@link #TTHREAD}
   * @see #typename()
   */
   ...
   * <p>
   *
   * @return name from type name list {@link #TYPE_NAMES} corresponding to the type of this value:
  ...
  /**
   * Check if {@code this} is a {@code function} that is a closure, meaning interprets lua bytecode
   * for its execution
   *
   ...
   *
   * <p>No attempt to convert from string will be made by this call.
   *
   ...
   *
   * @return true if this is a {@code number}, meaning derives from {@link LuaNumber} or derives
   *     from {@link LuaString} and is convertible to a number, otherwise false
   * @see #tonumber()
   * @see #checknumber()
   ...
   *
   * @return true if this is a {@code string}, meaning derives from {@link LuaString} or {@link
   *     LuaNumber}, otherwise false
   * @see #tostring()
   * @see #checkstring()
   ...
   * @return true if this is a {@code userdata} and the instance is assignable to {@code c},
   *     otherwise false
   * @see #isuserdata()
   ...
   *
   * @return userdata instance if is a userdata whose instance derives from {@code c}, or null if
   *     not {@link LuaUserdata}
   * @see #optuserdata(Class,Object)
   * @see #checkuserdata(Class)
   * @see #isuserdata(Class)
   * @see #TUSERDATA
   ...
   *
   * <p>In lua all numbers are strings, but not all strings are numbers. This function will return
   * the {@link LuaValue} {@code this} if it is a number or a string convertible to a number, and
   * {@link #NIL} for all other cases.
   *
   * <p>This allows values to be tested for their "numeric-ness" without the penalty of throwing
   * exceptions, nor the cost of converting the type and creating storage for it.
   *
   * @return {@code this} if it is a {@link LuaNumber} or {@link LuaString} that can be converted to
   *     a number, otherwise {@link #NIL}
   * @see #tostring()
   * @see #optnumber(LuaNumber)
   * @see #checknumber()
   * @see #toint()
   ...
   *
   * <p>In lua all numbers are strings, so this function will return the {@link LuaValue} {@code
   * this} if it is a string or number, and {@link #NIL} for all other cases.
   *
   * <p>This allows values to be tested for their "string-ness" without the penalty of throwing
   * exceptions.
   *
   ...
   * @return {@code this} cast to boolean if a {@link LuaBoolean}, {@code defval} if nil or none,
   ...
   *
   * <p>A {@link LuaClosure} is a {@link LuaFunction} that executes lua byteccode.
   *
   * @param defval {@link LuaClosure} to return if {@code this} is nil or none
   * @return {@code this} cast to {@link LuaClosure} if a function, {@code defval} if nil or none,
   *     throws {@link LuaError} otherwise
   ...
   * @return {@code this} cast to double if numeric, {@code defval} if nil or none, throws {@link
   *     LuaError} otherwise
   ...
   *
   * <p>A {@link LuaFunction} may either be a Java function that implements functionality directly
   * in Java, or a {@link LuaClosure} which is a {@link LuaFunction} that executes lua bytecode.
   *
   * @param defval {@link LuaFunction} to return if {@code this} is nil or none
   * @return {@code this} cast to {@link LuaFunction} if a function, {@code defval} if nil or none,
   *     throws {@link LuaError} otherwise
   ...
   * @return {@code this} cast to int if numeric, {@code defval} if nil or none, throws {@link
   *     LuaError} otherwise
   * @throws LuaError if was not numeric or nil or none.
   * @see #optdouble(double)
   * @see #optlong(long)
  ...
  /**
   * Check that optional argument is a number or string convertible to number and return as {@link
   * LuaInteger}
   *
   * @param defval {@link LuaInteger} to return if {@code this} is nil or none
   * @return {@code this} converted and wrapped in {@link LuaInteger} if numeric, {@code defval} if
   *     nil or none, throws {@link LuaError} otherwise
   * @throws LuaError if was not numeric or nil or none.
   * @see #optdouble(double)
   * @see #optint(int)
   * @see #checkint()
   ...
   * @return {@code this} cast to long if numeric, {@code defval} if nil or none, throws {@link
   *     LuaError} otherwise
   * @throws LuaError if was not numeric or nil or none.
   * @see #optdouble(double)
   * @see #optint(int)
   ...
   * @return {@code this} cast to {@link LuaNumber} if numeric, {@code defval} if nil or none,
   *     throws {@link LuaError} otherwise
   * @throws LuaError if was not numeric or nil or none.
   * @see #optdouble(double)
   * @see #optlong(long)
   ...
   * @return {@code this} converted to String if a string or number, {@code defval} if nil or none,
   ...
   *
   * @param defval {@link LuaString} to return if {@code this} is nil or none
   * @return {@code this} converted to {@link LuaString} if a string or number, {@code defval} if
   *     nil or none, throws {@link LuaError} if some other type
   * @throws LuaError if was not a string or number or nil or none.
   * @see #tojstring()
   * @see #optjstring(String)
   * @see #checkstring()
   ...
   * @return {@code this} cast to {@link LuaTable} if a thread, {@code defval} if nil or none,
   *     throws {@link LuaError} if some other type
   ...
   * @return Object instance of the userdata if a {@link LuaUserdata}, {@code defval} if nil or
  ...
  /**
   * Check that optional argument is a userdata whose instance is of a type and return the Object
   * instance
   *
   ...
   * @throws LuaError if was not a userdata whose instance is assignable to {@code c} or nil or
   *     none.
   * @see #checkuserdata(Class)
   * @see #isuserdata(Class)
   * @see #optuserdata(Object)
   ...
   *
   * <p>{@link LuaClosure} is a subclass of {@link LuaFunction} that interprets lua bytecode.
   *
  ...
  /**
   * Check that the value is numeric and return the value as a double, or throw {@link LuaError} if
   * not numeric
   *
   * <p>Values that are {@link LuaNumber} and values that are {@link LuaString} that can be
   * converted to a number will be converted to double.
   *
   * @return value cast to a double if numeric
   * @throws LuaError if not a {@link LuaNumber} or is a {@link LuaString} that can't be converted
   *     to number
   * @see #checkint()
   * @see #checkinteger()
   * @see #checklong()
   ...
   *
   * <p>{@link Globals} are a special {@link LuaTable} that establish the default global
   * environment.
   *
  ...
  /**
   * Check that the value is numeric, and convert and cast value to int, or throw {@link LuaError}
   * if not numeric
   *
   * <p>Values that are {@link LuaNumber} will be cast to int and may lose precision. Values that
   * are {@link LuaString} that can be converted to a number will be converted, then cast to int, so
  ...
  /**
   * Check that the value is numeric, and convert and cast value to long, or throw {@link LuaError}
   * if not numeric
   *
   * <p>Values that are {@link LuaNumber} will be cast to long and may lose precision. Values that
   * are {@link LuaString} that can be converted to a number will be converted, then cast to long,
   * so may also lose precision.
   *
   ...
   *
   * <p>Values that are {@link LuaString} that can be converted to a number will be converted and
   * returned.
   *
   ...
   *
   * <p>The string representations here will roughly match what is produced by the C lua
   * distribution, however hash codes have no relationship, and there may be differences in number
   * formatting.
   *
   ...
   *
   * <p>In lua all numbers are strings, so this will succeed for anything that derives from {@link
   * LuaString} or {@link LuaNumber}. Numbers will be converted to {@link LuaString}.
   *
   * @return {@link LuaString} representation of the value if it is a {@link LuaString} or {@link
   *     LuaNumber}
  ...
  /**
   * Assert a condition is true, or throw a {@link LuaError} if not Returns no value when b is true,
   * throws {@link #error(String)} with {@code msg} as argument and does not return if b is false.
   *
  ...
  /**
   * Throw a {@link LuaError} indicating an illegal operation occurred, typically involved in
  ...
  /**
   * Throw a {@link LuaError} based on an arithmetic error such as add, or pow, typically due to an
   ...
   *
   * @param rhs String description of what was on the right-hand-side of the comparison that
   ...
   * @throws LuaError if {@code this} is not a table, or there is no {@link #INDEX} metatag, or key
   *     is {@link #NIL}
   * @see #get(int)
   * @see #get(String)
   * @see #rawget(LuaValue)
   ...
   * @throws LuaError if {@code this} is not a table, or key is {@link #NIL}, or there is no {@link
   *     #NEWINDEX} metatag
   */
   ...
   *
   * <p>Primarily used internally in response to a SETLIST bytecode.
   *
  ...
  /**
   * Find the next key,value pair if {@code this} is a table, return {@link #NIL} if there are no
   * more, or throw a {@link LuaError} if not a table.
   *
   * <p>To iterate over all key-value pairs in a table you can use
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param index {@link LuaInteger} value identifying a key to start from, or {@link #NIL} to start
   *     at the beginning
   * @return {@link Varargs} containing {key,value} for the next entry, or {@link #NIL} if there are
   *     no more.
  ...
  /**
   * Find the next integer-key,value pair if {@code this} is a table, return {@link #NIL} if there
   * are no more, or throw a {@link LuaError} if not a table.
   *
   * <p>To iterate over integer keys in a table you can use
   *
   * <pre>{@code
   * LuaValue k = LuaValue.NIL;
   * while ( true ) {
   ...
   * @param index {@link LuaInteger} value identifying a key to start from, or {@link #NIL} to start
   *     at the beginning
   * @return {@link Varargs} containing {@code (key,value)} for the next entry, or {@link #NONE} if
   *     there are no more.
   * @throws LuaError if {@code this} is not a table, or the supplied key is invalid.
   * @see LuaTable
   * @see #next(LuaValue)
  ...
  /**
   * Load a library instance by calling it with and empty string as the modname, and this Globals as
   * the environment. This is normally used to iniitalize the library instance and which may install
   * itself into these globals.
   *
   ...
   *
   * <p>For {@link LuaTable} and {@link LuaUserdata} instances, the metatable returned is this
   * instance metatable. For all other types, the class metatable value will be returned.
   *
   ...
   *
   * <p>For {@link LuaTable} and {@link LuaUserdata} instances, the metatable is per instance. For
   * all other types, there is one metatable per type that can be set directly from java
   *
  ...
  /**
   * Call {@code this} with 0 arguments, including metatag processing, and return only the first
   * return value.
   *
   * <p>If {@code this} is a {@link LuaFunction}, call it, and return only its first return value,
   * dropping any others. Otherwise, look for the {@link #CALL} metatag and call that.
   *
   * <p>If the return value is a {@link Varargs}, only the 1st value will be returned. To get
   * multiple values, use {@link #invoke()} instead.
   *
   * <p>To call {@code this} as a method call, use {@link #method(LuaValue)} instead.
   *
   * @return First return value {@code (this())}, or {@link #NIL} if there were none.
   * @throws LuaError if not a function and {@link #CALL} is not defined, or the invoked function
   *     throws a {@link LuaError} or the invoked closure throw a lua {@code error}
   * @see #call(LuaValue)
  ...
  /**
   * Call {@code this} with 1 argument, including metatag processing, and return only the first
   * return value.
   *
   * <p>If {@code this} is a {@link LuaFunction}, call it, and return only its first return value,
   * dropping any others. Otherwise, look for the {@link #CALL} metatag and call that.
  ...
  /**
   * Call {@code this} with 2 arguments, including metatag processing, and return only the first
   * return value.
   *
   * <p>If {@code this} is a {@link LuaFunction}, call it, and return only its first return value,
   * dropping any others. Otherwise, look for the {@link #CALL} metatag and call that.
  ...
  /**
   * Call {@code this} with 3 arguments, including metatag processing, and return only the first
   * return value.
   *
   * <p>If {@code this} is a {@link LuaFunction}, call it, and return only its first return value,
   * dropping any others. Otherwise, look for the {@link #CALL} metatag and call that.
  ...
  /**
   * Call named method on {@code this} with 0 arguments, including metatag processing, and return
   * only the first return value.
   *
   * <p>Look up {@code this[name]} and if it is a {@link LuaFunction}, call it inserting {@code
   * this} as an additional first argument. and return only its first return value, dropping any
   * others. Otherwise, look for the {@link #CALL} metatag and call that.
   *
   * <p>If the return value is a {@link Varargs}, only the 1st value will be returned. To get
   * multiple values, use {@link #invoke()} instead.
   *
   * <p>To call {@code this} as a plain call, use {@link #call()} instead.
   *
  ...
  /**
   * Call named method on {@code this} with 1 argument, including metatag processing, and return
   * only the first return value.
   *
   * <p>Look up {@code this[name]} and if it is a {@link LuaFunction}, call it inserting {@code
   * this} as an additional first argument, and return only its first return value, dropping any
   ...
   * multiple values, use {@link #invoke()} instead.
   *
   * <p>To call {@code this} as a plain call, use {@link #call(LuaValue)} instead.
   *
   * @param name Name of the method to look up for invocation
  ...
  /**
   * Call named method on {@code this} with 2 arguments, including metatag processing, and return
   * only the first return value.
   *
   * <p>Look up {@code this[name]} and if it is a {@link LuaFunction}, call it inserting {@code
   * this} as an additional first argument, and return only its first return value, dropping any
   ...
   * multiple values, use {@link #invoke()} instead.
   *
   * <p>To call {@code this} as a plain call, use {@link #call(LuaValue,LuaValue)} instead.
   *
   * @param name Name of the method to look up for invocation
  ...
  /**
   * Call {@code this} with 0 arguments, including metatag processing, and retain all return values
   * in a {@link Varargs}.
   *
   * <p>If {@code this} is a {@link LuaFunction}, call it, and return all values. Otherwise, look
   * for the {@link #CALL} metatag and call that.
   *
   * <p>To get a particular return value, us {@link Varargs#arg(int)}
   *
   * <p>To call {@code this} as a method call, use {@link #invokemethod(LuaValue)} instead.
   *
  ...
  /**
   * Call {@code this} with variable arguments, including metatag processing, and retain all return
   ...
   * <p>To get a particular return value, us {@link Varargs#arg(int)}
   *
   * <p>To call {@code this} as a method call, use {@link #invokemethod(LuaValue,Varargs)} instead.
   *
  ...
  /**
   * Call named method on {@code this} with 0 arguments, including metatag processing, and retain
   * all return values in a {@link Varargs}.
   *
   * <p>Look up {@code this[name]} and if it is a {@link LuaFunction}, call it inserting {@code
   * this} as an additional first argument, and return all return values as a {@link Varargs}
   * instance. Otherwise, look for the {@link #CALL} metatag and call that.
   *
   * <p>To get a particular return value, us {@link Varargs#arg(int)}
   *
   * <p>To call {@code this} as a plain call, use {@link #invoke()} instead.
   *
   * @param name Name of the method to look up for invocation
   * @return All values returned from {@code this:name()} as a {@link Varargs} instance
   * @throws LuaError if not a function and {@link #CALL} is not defined, or the invoked function
  ...
  /**
   * Call named method on {@code this} with 1 argument, including metatag processing, and retain all
   * return values in a {@link Varargs}.
   *
   * <p>Look up {@code this[name]} and if it is a {@link LuaFunction}, call it inserting {@code
   * this} as an additional first argument, and return all return values as a {@link Varargs}
   * instance. Otherwise, look for the {@link #CALL} metatag and call that.
   ...
   * <p>To get a particular return value, us {@link Varargs#arg(int)}
   *
   * <p>To call {@code this} as a plain call, use {@link #invoke(Varargs)} instead.
   *
   * @param name Name of the method to look up for invocation
   * @param args {@link Varargs} containing arguments to supply to the called function after {@code
   *     this}
  ...
  /**
   * Call named method on {@code this} with variable arguments, including metatag processing, and
   ...
   *
   * @param name Name of the method to look up for invocation
   * @param args Array of {@link LuaValue} containing arguments to supply to the called function
   *     after {@code this}
   * @return All values returned from {@code this:name(args)} as a {@link Varargs} instance
   * @throws LuaError if not a function and {@link #CALL} is not defined, or the invoked function
   *     throws a {@link LuaError} or the invoked closure throw a lua {@code error}
   ...
   *
   * @return boolean inverse as {@link LuaBoolean} if boolean or nil, numeric inverse as {@link
   *     LuaNumber} if numeric, or metatag processing result if {@link #UNM} metatag is defined
  ...
  /**
   * Length operator: return lua length of object {@code (#this)} including metatag processing as
   * java int
   *
   ...
   * java int
   *
   * @return length as defined by the lua # operator or metatag processing result converted to java
   *     int using {@link #toint()}
   * @throws LuaError if {@code this} is not a table or string, and has no {@link #LEN} metatag
   */
  public int length() {
    return len().toint();
   ...
   * @return {@link #TRUE} if values are comparable and {@code (this == rhs)}, {@link #FALSE} if
   *     comparable but not equal, {@link LuaValue} if metatag processing occurs.
   * @see #eq_b(LuaValue)
   * @see #raweq(LuaValue)
   * @see #neq(LuaValue)
  ...
  /**
   * Equals: Perform equality comparison with another value including metatag processing using
   * {@link #EQ}, and return java boolean
   *
   * @param val The value to compare with.
   * @return true if values are comparable and {@code (this == rhs)}, false if comparable but not
   *     equal, result converted to java boolean if metatag processing occurs.
   * @see #eq(LuaValue)
   * @see #raweq(LuaValue)
   * @see #neq_b(LuaValue)
   * @see #eqmtcall(LuaValue, LuaValue, LuaValue, LuaValue)
   ...
   * @param val The value to compare with.
   * @return {@link #TRUE} if values are comparable and {@code (this != rhs)}, {@link #FALSE} if
   *     comparable but equal, inverse of {@link LuaValue} converted to {@link LuaBoolean} if
   ...
   *
   * @param val The value to compare with.
   * @return true if values are comparable and {@code (this != rhs)}, false if comparable but equal,
  ...
  /**
   * Equals: Perform direct equality comparison with a {@link LuaUserdata} value without metatag
   * processing.
   *
   * @param val The {@link LuaUserdata} to compare with.
   * @return true if {@code this} is userdata and their metatables are the same using == and their
   *     instances are equal using {@link #equals(Object)}, otherwise false
   * @see #eq(LuaValue)
   * @see #raweq(LuaValue)
   */
  ...
  /**
   * Equals: Perform direct equality comparison with a {@link LuaString} value without metatag
   * processing.
   *
   * @param val The {@link LuaString} to compare with.
   * @return true if {@code this} is a {@link LuaString} and their byte sequences match, otherwise
   *     false
   */
   ...
   *
   * <p>Each operand must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   *
   * @param rhs The right-hand-side value to perform the add with
   * @return value of {@code (this + rhs)} if both are numeric, or {@link LuaValue} if metatag
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #ADD} metatag defined
   ...
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   *
   * @param rhs The right-hand-side value to perform the add with
  ...
  /**
   * Subtract: Perform numeric subtract operation with another value of unknown type, including
   * metatag processing.
   *
   * <p>Each operand must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #SUB} metatag defined
   * @see #arithmt(LuaValue, LuaValue)
   */
  ...
  /**
   * Subtract: Perform numeric subtract operation with another value of double type with metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
  ...
  /**
   * Subtract: Perform numeric subtract operation with another value of int type with metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
  ...
  /**
   * Reverse-subtract: Perform numeric subtract operation from a double value without metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   *
   * <p>For metatag processing {@link #sub(LuaValue)} must be used
   *
   * @param lhs The left-hand-side value from which to perform the subtraction
   * @return value of {@code (lhs - this)} if this is numeric
   * @throws LuaError if {@code this} is not a number or string convertible to number
  ...
  /**
   * Multiply: Perform numeric multiply operation with another value of unknown type, including
   * metatag processing.
   *
   * <p>Each operand must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #MUL} metatag defined
   * @see #arithmt(LuaValue, LuaValue)
   */
  ...
  /**
   * Multiply: Perform numeric multiply operation with another value of double type with metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
  ...
  /**
   * Multiply: Perform numeric multiply operation with another value of int type with metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #POW} metatag defined
   * @see #arithmt(LuaValue, LuaValue)
   */
  ...
  /**
   * Reverse-raise to power: Raise another value of double type to this power with metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #DIV} metatag defined
   * @see #arithmt(LuaValue, LuaValue)
   */
  ...
  /**
   * Divide: Perform numeric divide operation by another value of double type without metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   *
   * <p>For metatag processing {@link #div(LuaValue)} must be used
   *
   * @param rhs The right-hand-side value to perform the divulo with
   * @return value of {@code (this / rhs)} if this is numeric
   * @throws LuaError if {@code this} is not a number or string convertible to number
  ...
  /**
   * Divide: Perform numeric divide operation by another value of int type without metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *     processing occurs
   * @throws LuaError if either operand is not a number or string convertible to number, and neither
   *     has the {@link #MOD} metatag defined
   * @see #arithmt(LuaValue, LuaValue)
   */
  ...
  /**
   * Modulo: Perform numeric modulo operation with another value of double type without metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   *
   * <p>For metatag processing {@link #mod(LuaValue)} must be used
   *
   * @param rhs The right-hand-side value to perform the modulo with
   * @return value of {@code (this % rhs)} if this is numeric
   * @throws LuaError if {@code this} is not a number or string convertible to number
  ...
  /**
   * Modulo: Perform numeric modulo operation with another value of int type without metatag
   * processing
   *
   * <p>{@code this} must derive from {@link LuaNumber} or derive from {@link LuaString} and be
   * convertible to a number
   ...
   *
   * <p>Finds the supplied metatag value for {@code this} or {@code op2} and invokes it, or throws
   ...
   *
   * <p>Finds the supplied metatag value for {@code this} and invokes it, or throws {@link LuaError}
  ...
  protected LuaValue arithmtwith(LuaValue tag, double op1) {
    ...
    if (h.isnil()) error("attempt to perform arithmetic " + tag + " on number and " + typename());
    return h.call(LuaValue.valueOf(op1), this);
  }

  /**
   * Less than: Perform numeric or string comparison with another value of unknown type, including
   * metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, both operands must derive from {@link LuaString} or both must derive from
   * {@link LuaNumber}.
   *
   * @param rhs The right-hand-side value to perform the comparison with
   * @return {@link #TRUE} if {@code (this < rhs)}, {@link #FALSE} if not, or {@link LuaValue} if
   *     metatag processing occurs
   * @throws LuaError if either both operands are not a strings or both are not numbers and no
   *     {@link #LT} metatag is defined.
   * @see #gteq_b(LuaValue)
  ...
  /**
   * Less than: Perform numeric comparison with another value of double type, including metatag
   * processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
   * @param rhs The right-hand-side value to perform the comparison with
   * @return {@link #TRUE} if {@code (this < rhs)}, {@link #FALSE} if not, or {@link LuaValue} if
   *     metatag processing occurs
   * @throws LuaError if this is not a number and no {@link #LT} metatag is defined.
   * @see #gteq_b(double)
   * @see #comparemt(LuaValue, LuaValue)
   */
  ...
  /**
   * Less than: Perform numeric comparison with another value of int type, including metatag
   * processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
   ...
   *
   * @param rhs The right-hand-side value to perform the comparison with
   * @return true if {@code (this < rhs)}, false if not, and boolean interpreation of result if
   *     metatag processing occurs.
   * @throws LuaError if either both operands are not a strings or both are not numbers and no
   *     {@link #LT} metatag is defined.
   * @see #gteq(LuaValue)
  ...
  /**
   * Less than or equals: Perform numeric or string comparison with another value of unknown type,
  ...
  /**
   * Less than or equals: Perform numeric comparison with another value of double type, including
   * metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
   ...
   * @return {@link #TRUE} if {@code (this <= rhs)}, {@link #FALSE} if not, or {@link LuaValue} if
   *     metatag processing occurs
   * @throws LuaError if this is not a number and no {@link #LE} metatag is defined.
   * @see #gteq_b(double)
   * @see #comparemt(LuaValue, LuaValue)
   */
  ...
  /**
   * Less than or equals: Perform numeric comparison with another value of int type, including
   * metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
  ...
  /**
   * Greater than: Perform numeric or string comparison with another value of unknown type,
   * including metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, both operands must derive from {@link LuaString} or both must derive from
   * {@link LuaNumber}.
  ...
  /**
   * Greater than: Perform numeric comparison with another value of int type, including metatag
   * processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
  ...
  /**
   * Greater than or equals: Perform numeric or string comparison with another value of unknown
  ...
  /**
   * Greater than or equals: Perform numeric comparison with another value of double type, including
   * metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
  ...
  /**
   * Greater than or equals: Perform numeric comparison with another value of int type, including
   * metatag processing, and returning {@link LuaValue}.
   *
   * <p>To be comparable, this must derive from {@link LuaNumber}.
   *
   ...
   *
   * <p>Finds the supplied metatag value and invokes it, or throws {@link LuaError} if none applies.
   *
   * @param tag The metatag to look up
   * @param op1 The operand with which to to perform the operation
   * @return {@link LuaValue} resulting from metatag processing
   * @throws LuaError if metatag was not defined for either operand, or if the operands are not the
   *     same type, or the metatag values for the two operands are different.
   * @see #gt(LuaValue)
   * @see #gteq(LuaValue)
   * @see #lt(LuaValue)
   * @see #lteq(LuaValue)
  ...
  public LuaValue comparemt(LuaValue tag, LuaValue op1) {
    LuaValue h;
    if (!(h = metatag(tag)).isnil() || !(h = op1.metatag(tag)).isnil()) return h.call(this, op1);
  ...
  /**
   * Perform string comparison with another value of any type using string comparison based on byte
   * values.
   *
   * <p>Only strings can be compared, meaning each operand must derive from {@link LuaString}.
   *
   * @param rhs The right-hand-side value to perform the comparison with
   * @return int < 0 for {@code (this < rhs)}, int > 0 for {@code (this > rhs)}, or 0 when same
   *     string.
  ...
  /**
   * Perform string comparison with another value known to be a {@link LuaString} using string
  ...
  /**
   * Concatenate another value onto this value and return the result using rules of lua string
   * concatenation including metatag processing.
   *
   * <p>Only strings and numbers as represented can be concatenated, meaning each operand must
   * derive from {@link LuaString} or {@link LuaNumber}.
   *
  ...
  /**
   * Reverse-concatenation: concatenate this value onto another value whose type is unknwon and
  ...
  /**
   * Reverse-concatenation: concatenate this value onto another value known to be a {@link
   * LuaNumber} and return the result using rules of lua string concatenation including metatag
   * processing.
   *
   * <p>Only strings and numbers as represented can be concatenated, meaning each operand must
   * derive from {@link LuaString} or {@link LuaNumber}.
  ...
  /**
   * Reverse-concatenation: concatenate this value onto another value known to be a {@link
   * LuaString} and return the result using rules of lua string concatenation including metatag
   * processing.
   *
   * <p>Only strings and numbers as represented can be concatenated, meaning each operand must
   * derive from {@link LuaString} or {@link LuaNumber}.
   ...
   *
   * <p>Finds the {@link #CONCAT} metatag value and invokes it, or throws {@link LuaError} if it
   * doesn't exist.
   *
   * @param rhs The right-hand-side value to perform the operation with
   ...
   * This returns either {@code this} or {@code rhs} depending on the boolean value for {@code
   * this}.
   *
   * @param rhs The right-hand-side value to perform the operation with
   ...
   *
   * <p>Used in lua-bytecode to Java-bytecode conversion.
   *
  ...
  /**
   * Convert java double to a {@link LuaValue}. This may return a {@link LuaInteger} or {@link
   * LuaDouble} depending on the value supplied.
   *
   ...
   * @return {@link LuaString} instance, possibly pooled, whose bytes are those in the supplied
   *     array
   */
   ...
   * @return new {@link LuaTable} instance with no values and no metatable, but preallocated for
   *     array and hashed elements.
   */
   ...
   * Construct a {@link LuaTable} initialized with supplied array values.
   *
   * @param unnamedValues array of {@link LuaValue} containing the first values to use in
   *     initialization
   * @param lastarg {@link Varargs} containing additional values to use in initialization to be put
   *     after the last unnamedValues element
   * @return new {@link LuaTable} instance with sequential elements coming from the array and
   *     varargs.
   */
   ...
   *
   * @param namedValues array of {@link LuaValue} containing the keys and values to use in
   *     initialization in order {@code {key-a, value-a, key-b, value-b, ...} }
  ...
  /**
   * Construct a {@link LuaTable} initialized with supplied named values and sequential elements in
   * an array part and as varargs. The named values will be assigned first, and the sequential
   * elements will be assigned later, possibly overwriting named values at the same slot if there
   * are conflicts.
   *
   * @param namedValues array of {@link LuaValue} containing the keys and values to use in
   *     initialization in order {@code {key-a, value-a, key-b, value-b, ...} }
   * @param unnamedValues array of {@link LuaValue} containing the first sequenctial elements to use
   *     in initialization in order {@code {value-1, value-2, ...} }, or null if there are none
   * @param lastarg {@link Varargs} containing additional values to use in the sequential part of
   *     the initialization, to be put after the last unnamedValues element
   * @return new {@link LuaTable} instance with named and sequential values supplied.
   */
  public static LuaTable tableOf(
      LuaValue[] namedValues, LuaValue[] unnamedValues, Varargs lastarg) {
  ...
  /**
   * Return value for field reference including metatag processing, or {@link LuaValue#NIL} if it
   * doesn't exist.
   *
   * @param t {@link LuaValue} on which field is being referenced, typically a table or something
   *     with the metatag {@link LuaValue#INDEX} defined
  ...
  protected static LuaValue gettable(LuaValue t, LuaValue key) {
    ...
    do {
      ...
      } else if ((tm = t.metatag(INDEX)).isnil()) t.indexerror(key.tojstring());
   ...
   *
   * @param t {@link LuaValue} on which value is being set, typically a table or something with the
   *     metatag {@link LuaValue#NEWINDEX} defined
  ...
  protected static boolean settable(LuaValue t, LuaValue key, LuaValue value) {
    ...
    do {
      if (t.istable()) {
        ...
        throw new LuaError(
            "table expected for set index ('" + key + "') value, got " + t.typename());
      if (tm.isfunction()) {
        tm.call(t, key, value);
        return true;
      }
   ...
   *
   * @param tag Metatag name to look up, typically a string such as {@link LuaValue#INDEX} or {@link
   *     LuaValue#NEWINDEX}
  ...
  protected LuaValue checkmetatag(LuaValue tag, String reason) {
    LuaValue h = this.metatag(tag);
    if (h.isnil()) throw new LuaError(reason + "a " + typename() + " value");
    return h;
  }
   ...
   * Construct a {@link Varargs} around an array of {@link LuaValue}s.
   *
   * <p>Caller must ensure that array contents are not mutated after this call or undefined behavior
   * will result.
   *
   * @param v The array of {@link LuaValue}s
   * @param offset number of initial values to skip in the array
   ...
   * @see LuaValue#varargsOf(LuaValue[], int, int)
   */
  public static Varargs varargsOf(
      final LuaValue[] v, final int offset, final int length, Varargs more) {
    switch (length) {
      case 0:
        return more;
      case 1:
   ...
   *
   * <p>This can be used to wrap exactly 2 values, or a list consisting of 1 initial value followed
   * by another variable list of remaining values.
   *
   * @param v First {@link LuaValue} in the {@link Varargs}
   * @param r {@link LuaValue} supplying the 2rd value, or {@link Varargs}s supplying all values
   *     beyond the first
   * @return {@link Varargs} wrapping the supplied values.
   */
   ...
   *
   * <p>This can be used to wrap exactly 3 values, or a list consisting of 2 initial values followed
   * by another variable list of remaining values.
   *
   * @param v1 First {@link LuaValue} in the {@link Varargs}
   * @param v2 Second {@link LuaValue} in the {@link Varargs}
   * @param v3 {@link LuaValue} supplying the 3rd value, or {@link Varargs}s supplying all values
   *     beyond the second
   * @return {@link Varargs} wrapping the supplied values.
   */
   ...
   *
   * <p>The tail call is not yet called or processing until the client invokes {@link
   * TailcallVarargs#eval()} which performs the tail call processing.
   *
   * <p>This method is typically not used directly by client code. Instead use one of the function
   * invocation methods.
   *
   ...
   *
   * <p>This may return a {@link TailcallVarargs} to be evaluated by the client.
   *
   * <p>This should not be called directly, instead use one of the call invocation functions.
   *
  ...
  /**
   * Hook for implementations such as LuaJC to load the environment of the main chunk into the first
   * upvalue location. If the function has no upvalues or is not a main chunk, calling this will be
   * no effect.
   *
   ...
   *
   * <p>This is an internal class not intended to be used directly. Instead use the predefined
   * constant {@link LuaValue#NONE}
   *
   * @see LuaValue#NONE
   */
