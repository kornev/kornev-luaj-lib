...
/**
 * Visitor that resolves names to scopes. Each Name is resolved to a NamedVarible, possibly in a
 * NameScope if it is a local, or in no named scope if it is a global.
 */
public class NameResolver extends Visitor {
  ...
  public void visit(LocalAssign stat) {
    ...
    if (!isvarlist)
      for (int i = m; i < n; i++) ((Name) stat.names.get(i)).variable.initialValue = LuaValue.NIL;
  }
  ...
  protected void defineLocalVars(List<Name> names) {
    for (int i = 0, n = names.size(); i < n; i++) defineLocalVar((Name) names.get(i));
  }
