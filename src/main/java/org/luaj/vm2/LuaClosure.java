 ...
 *
 * <p>A {@link LuaClosure} is a combination of a {@link Prototype} and a {@link LuaValue} to use as
 * an environment for execution. Normally the {@link LuaValue} is a {@link Globals} in which case
 * the environment will contain standard lua libraries.
 *
 * <p>There are three main ways {@link LuaClosure} instances are created:
 *
 * <ul>
 *   <li>Construct an instance using {@link #LuaClosure(Prototype, LuaValue)}
 ...
 * </ul>
 *
 * <p>To construct it directly, the {@link Prototype} is typically created via a compiler such as
 ...
 * }</pre>
 *
 * <p>To construct it indirectly, the {@link Globals#load(java.io.Reader, String)} method may be
 * used:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>In this example, the "checkclosure()" may fail if direct lua-to-java-bytecode compiling using
 * LuaJC is installed, because no LuaClosure is created in that case and the value returned is a
 * {@link LuaFunction} but not a {@link LuaClosure}.
 *
 * <p>Since a {@link LuaClosure} is a {@link LuaFunction} which is a {@link LuaValue}, all the value
 * operations can be used directly such as:
 *
 * <ul>
 *   <li>{@link LuaValue#call()}
 *   <li>{@link LuaValue#call(LuaValue)}
 *   <li>{@link LuaValue#invoke()}
 *   <li>{@link LuaValue#invoke(Varargs)}
 *   <li>{@link LuaValue#method(String)}
 *   <li>{@link LuaValue#method(String,LuaValue)}
 *   <li>{@link LuaValue#invokemethod(String)}
 *   <li>{@link LuaValue#invokemethod(String,Varargs)}
 *   <li>...
 * </ul>
 *
 * @see LuaValue
...
public class LuaClosure extends LuaFunction {
  ...
  /**
   * Create a closure around a Prototype with a specific environment. If the prototype has upvalues,
   * the environment will be written into the first upvalue.
   *
  ...
  public void initupvalue1(LuaValue env) {
    if (p.upvalues == null || p.upvalues.length == 0) this.upValues = NOUPVALUES;
    else {
  ...
  protected Varargs execute(LuaValue[] stack, Varargs varargs) {
    ...
    try {
      for (; true; ++pc) {
        ...
        switch (i & 0x3f) {
          ...
          case Lua.OP_LOADBOOL: /*	A B C	R(A):= (Bool)B: if (C) pc++			*/
            ...
            if ((i & (0x1ff << 14)) != 0) ++pc; /* skip next instruction (if C) */
            continue;

          case Lua.OP_LOADNIL: /*	A B	R(A):= ...:= R(A+B):= nil			*/
            for (b = i >>> 23; b-- >= 0; ) stack[a++] = LuaValue.NIL;
            continue;
          ...
          case Lua.OP_GETTABUP: /*	A B C	R(A) := UpValue[B][RK(C)]			*/
            stack[a] =
                upValues[i >>> 23]
                    .getValue()
                    .get((c = (i >> 14) & 0x1ff) > 0xff ? k[c & 0x0ff] : stack[c]);
            continue;

          case Lua.OP_GETTABLE: /*	A B C	R(A):= R(B)[RK(C)]				*/
            stack[a] =
                stack[i >>> 23].get((c = (i >> 14) & 0x1ff) > 0xff ? k[c & 0x0ff] : stack[c]);
            continue;
          ...
          case Lua.OP_EQ: /*	A B C	if ((RK(B) == RK(C)) ~= A) then pc++		*/
            if (((b = i >>> 23) > 0xff ? k[b & 0x0ff] : stack[b])
                    .eq_b((c = (i >> 14) & 0x1ff) > 0xff ? k[c & 0x0ff] : stack[c])
                != (a != 0)) ++pc;
            continue;

          case Lua.OP_LT: /*	A B C	if ((RK(B) <  RK(C)) ~= A) then pc++  		*/
            if (((b = i >>> 23) > 0xff ? k[b & 0x0ff] : stack[b])
                    .lt_b((c = (i >> 14) & 0x1ff) > 0xff ? k[c & 0x0ff] : stack[c])
                != (a != 0)) ++pc;
            continue;

          case Lua.OP_LE: /*	A B C	if ((RK(B) <= RK(C)) ~= A) then pc++  		*/
            if (((b = i >>> 23) > 0xff ? k[b & 0x0ff] : stack[b])
                    .lteq_b((c = (i >> 14) & 0x1ff) > 0xff ? k[c & 0x0ff] : stack[c])
                != (a != 0)) ++pc;
            continue;
          ...
          case Lua.OP_TAILCALL: /*	A B C	return R(A)(R(A+1), ... ,R(A+B-1))		*/
            switch (i & Lua.MASK_B) {
              ...
              case (4 << Lua.POS_B):
                return new TailcallVarargs(
                    stack[a], varargsOf(stack[a + 1], stack[a + 2], stack[a + 3]));
              default:
                b = i >>> 23;
                v =
                    b > 0
          ...
          case Lua.OP_SETLIST: /*	A B C	R(A)[(C-1)*FPF+i]:= R(A+i), 1 <= i <= B	*/
            {
              ...
              if ((b = i >>> 23) == 0) {
                ...
                int m = b - v.narg();
                int j = 1;
                for (; j <= m; j++) o.set(offset + j, stack[a + j]);
                for (; j <= b; j++) o.set(offset + j, v.arg(j - m));
              } else {
                o.presize(offset + b);
                for (int j = 1; j <= b; j++) o.set(offset + j, stack[a + j]);
              }
            }
            continue;
          ...
          case Lua.OP_VARARG: /*	A B	R(A), R(A+1), ..., R(A+B-1) = vararg		*/
            ...
            if (b == 0) {
              ...
              for (int j = 1; j < b; ++j) stack[a + j - 1] = varargs.arg(j);
            }
            continue;
      ...
      if (openups != null)
        for (int u = openups.length; --u >= 0; ) if (openups[u] != null) openups[u].close();
  ...
  private UpValue findupval(LuaValue[] stack, short idx, UpValue[] openups) {
    ...
    for (int i = 0; i < n; ++i)
      if (openups[i] != null && openups[i].index == idx) return openups[i];
    for (int i = 0; i < n; ++i) if (openups[i] == null) return openups[i] = new UpValue(stack, idx);
