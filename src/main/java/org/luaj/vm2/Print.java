...
public class Print extends Lua {
    ...
    if (o >= OPNAMES.length - 1) {
      ...
      switch (o) {
        ...
        case OP_CLOSURE:
          ...
          break;
        case OP_SETLIST:
          if (c == 0) ps.print("  ; " + ((int) code[++pc]) + " (stored in the next OP)");
          else ps.print("  ; " + ((int) c));
          break;
        case OP_VARARG:
  ...
  static void printHeader(Prototype f) {
    ...
    ps.print(
        "\n%"
            ...
            + ")\n");
    ps.print(
        f.numparams + " param, " + f.maxstacksize + " slot, " + f.upvalues.length + " upvalue, ");
    ps.print(
        f.locvars.length + " local, " + f.k.length + " constant, " + f.p.length + " function\n");
  }
  ...
  public static void printFunction(Prototype prototype, boolean full) {
    ...
    for (i = 0; i < n; i++) printFunction(prototype.p[i], full);
  }
  ...
  private static void format(String s, int maxcols) {
    ...
    if (n > maxcols) ps.print(s.substring(0, maxcols));
      ...
      ps.print(s);
      for (int i = maxcols - n; --i >= 0; ) ps.print(' ');
    }
  }
