 ...
 *
 * Typically, this is constructed indirectly by a call to {@link
 * org.luaj.vm2.lib.jse.JsePlatform#standardGlobals()} and then used to load lua scripts for
 ...
 *
 * <p>For specialized circumstances, the Globals may be constructed directly and loaded with only
 * those libraries that are needed, for example.
 *
 * <pre>{@code
 ...
 *
 * Globals contains convenience functions to load and execute lua source code given a Reader. A
 * simple example is:
 *
 * <pre>{@code
 * globals.load(new StringReader("print 'hello'"), "main.lua").call();
 ...
 *   <li>construct {@link LuaClosure} from {@link Prototype} with {@link Globals} using {@link
 *       Loader}
 * </ul>
 *
 * <p>There are alternate flows when the direct lua-to-Java bytecode compiling {@link
 * org.luaj.vm2.luajc.LuaJC} is used.
 *
 * <ul>
 *   <li>compile lua to lua bytecode using {@link Compiler} or load precompiled code using {@link
 *       Undumper}
 *   <li>convert lua bytecode to equivalent Java bytecode using {@link org.luaj.vm2.luajc.LuaJC}
 *       that implements {@link Loader} directly
 * </ul>
 *
 * <h3>Java Field</h3>
 *
 ...
 *
 * When using {@link org.luaj.vm2.lib.jse.JsePlatform}, these environment variables are created
 * within the Globals.
 *
 * <ul>
 ...
 * which will be logically distinct and not interfere with each other, but share certain static
 * immutable resources such as class data and string data.
 *
 * <p>
 *
...
public class Globals extends LuaTable {
  ...
  /**
   * Convenience function to load a string value as a script with a custom environment. Must be lua
   * source.
   *
   * @param script Contents of a lua script, such as "print 'hello, world.'"
   * @param chunkname Name that will be used within the chunk as the source.
  ...
  /**
   * Load the content form a reader as a text file. Must be lua source. The source is converted to
   * UTF-8, so any characters appearing in quoted literals above the range 128 will be converted
   * into multiple bytes.
   *
  ...
  /**
   * Load the content form a reader as a text file, supplying a custom environment. Must be lua
   * source. The source is converted to UTF-8, so any characters appearing in quoted literals above
   * the range 128 will be converted into multiple bytes.
   *
   * @param reader Reader containing text of a lua script, such as "print 'hello, world.'"
   * @param chunkname Name that will be used within the chunk as the source.
   * @param environment LuaTable to be used as the environment for the loaded function.
   ...
   * @param chunkname Name that will be used within the chunk as the source.
   * @param mode String containing 'b' or 't' or both to control loading as binary or text or
   *     either.
   * @param environment LuaTable to be used as the environment for the loaded function.
   */
  ...
  /**
   * Load lua source or lua binary from an input stream into a Prototype. The InputStream is either
   * a binary lua chunk starting with the lua binary chunk signature, or a text input file. If it is
   * a text input file, it is interpreted as a UTF-8 byte sequence.
   *
  ...
  public Prototype loadPrototype(InputStream is, String chunkname, String mode) throws IOException {
    if (mode.indexOf('b') >= 0) {
      ...
      if (!is.markSupported()) is = new BufferedStream(is);
      is.mark(4);
  ...
  /**
   * Compile lua source from a Reader into a Prototype. The characters in the reader are converted
   * to bytes using the UTF-8 encoding, so a string literal containing characters with codepoints
   * 128 or above will be converted into multiple bytes.
   */
  ...
  /**
   * Compile lua source from an InputStream into a Prototype. The input is assumed to be UTf-8, but
   * since bytes in the range 128-255 are passed along as literal bytes, any ASCII-compatible
   * encoding such as ISO 8859-1 may also be used.
   */
  ...
  public Varargs yield(Varargs args) {
    if (running == null || running.isMainThread()) throw new LuaError("cannot yield main thread");
  ...
  static class StrReader extends Reader {
    ...
    public int read(char[] cbuf, int off, int len) throws IOException {
      int j = 0;
      for (; j < len && i < n; ++j, ++i) cbuf[off + j] = s.charAt(i);
  ...
  /**
   * Simple converter from Reader to InputStream using UTF8 encoding that will work on both JME and
   * JSE. This class may be moved to its own package in the future.
   */
  ...
  /**
   * Simple buffered InputStream that supports mark. Used to examine an InputStream for a 4-byte
   * binary lua signature, and fall back to text input when the signature is not found, as well as
   * speed up normal compilation and reading of lua scripts. This class may be moved to its own
