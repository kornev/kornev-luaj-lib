...
/**
 * Subclass of {@link LibFunction} which implements the lua standard package and module library
 * functions.
 *
 ...
 *
 * <ul>
 *   <li><code>"luaj.package.path"</code> Initial value for <code>"package.path"</code>. Default
 *       value is <code>"?.lua"</code>
 * </ul>
 *
 * <h3>Loading</h3>
 *
 * Typically, this library is included as part of a call to {@link
 ...
 * <h3>Limitations</h3>
 *
 * This library has been implemented to match as closely as possible the behavior in the
 * corresponding library in C. However, the default filesystem search semantics are different and
 * delegated to the bas library as outlined in the {@link BaseLib} and {@link
 * org.luaj.vm2.lib.jse.JseBaseLib} documentation.
 *
 * <p>
 *
 * @see LibFunction
...
public class PackageLib extends TwoArgFunction {
  ...
  /**
   * The default value to use for package.path. This can be set with the system property <code>
   * "luaj.package.path"</code>, and is <code>"?.lua"</code> by default.
   */
  ...
  /**
   * Perform one-time initialization on the library by adding package functions to the supplied
   * environment, and returning it as the return value. It also creates the package.preload and
   * package.loaded tables for use by other libraries.
   *
  ...
  /**
   * Set the lua path used by this library instance to a new value. Merely sets the value of {@link
   * path} to be used in subsequent searches.
   */
   ...
   * require (modname)
   *
   * <p>Loads the given module. The function starts by looking into the package.loaded table to
   * determine whether modname is already loaded. If it is, then require returns the value stored at
   * package.loaded[modname]. Otherwise, it tries to find a loader for the module.
   *
   * <p>To find a loader, require is guided by the package.searchers sequence. By changing this
   * sequence, we can change how require looks for a module. The following explanation is based on
   * the default configuration for package.searchers.
   *
   * <p>First require queries package.preload[modname]. If it has a value, this value (which should
   * be a function) is the loader. Otherwise require searches for a Lua loader using the path stored
   * in package.path. If that also fails, it searches for a Java loader using the classpath, using
   * the public default constructor, and casting the instance to LuaFunction.
   *
   * <p>Once a loader is found, require calls the loader with two arguments: modname and an extra
   * value dependent on how it got the loader. If the loader came from a file, this extra value is
   * the file name. If the loader is a Java instance of LuaFunction, this extra value is the
   * environment. If the loader returns any non-nil value, require assigns the returned value to
   * package.loaded[modname]. If the loader does not return a non-nil value and has not assigned any
   * value to package.loaded[modname], then require assigns true to this entry. In any case, require
   * returns the final value of package.loaded[modname].
   *
   * <p>If there is any error loading or running the module, or if it cannot find any loader for the
   * module, then require raises an error.
   */
  public class require extends OneArgFunction {
    public LuaValue call(LuaValue arg) {
      ...
      if (result.toboolean()) {
        if (result == _SENTINEL) error("loop or previous error loading module '" + name + "'");
        return result;
      }
  ...
  public class java_searcher extends VarArgFunction {
    public Varargs invoke(Varargs args) {
      ...
      try {
        ...
        if (v.isfunction()) ((LuaFunction) v).initupvalue1(globals);
