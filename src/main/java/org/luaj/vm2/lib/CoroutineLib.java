 ...
 *
 * <p>The coroutine library in luaj has the same behavior as the coroutine library in C, but is
 * implemented using Java Threads to maintain the call state between invocations. Therefore it can
 * be yielded from anywhere, similar to the "Coco" yield-from-anywhere patch available for C-based
 * lua. However, coroutines that are yielded but never resumed to complete their execution may not
