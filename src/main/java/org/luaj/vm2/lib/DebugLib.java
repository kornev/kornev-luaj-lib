 ...
 *
 * <p>The debug library in luaj tries to emulate the behavior of the corresponding C-based lua
 * library. To do this, it must maintain a separate stack of calls to {@link LuaClosure} and {@link
 * LibFunction} instances. Especially when lua-to-java bytecode compiling is being used via a {@link
 * org.luaj.vm2.Globals.Compiler} such as {@link org.luaj.vm2.luajc.LuaJC}, this cannot be done in
 * all cases.
 *
 ...
 * }</pre>
 *
 * <p>This library exposes the entire state of lua code, and provides method to see and modify all
 * underlying lua values within a Java VM so should not be exposed to client code in a shared server
 * environment.
 *
 * @see LibFunction
...
public class DebugLib extends TwoArgFunction {
  ...
  final class getinfo extends VarArgFunction {
    public Varargs invoke(Varargs args) {
      ...
      if (what.indexOf('f') >= 0) {
        if (func != null) info.set(FUNC, func);
      }
      return info;
    }
  }
  ...
  public void onInstruction(int pc, Varargs v, int top) {
    ...
    if (s.hookcount > 0) if (++s.bytecodes % s.hookcount == 0) callHook(s, COUNT, NIL);
    if (s.hookline) {
  ...
  CallStack callstack(LuaThread t) {
    if (t.callstack == null) t.callstack = new CallStack();
  ...
  public static class CallStack {
    ...
    private synchronized CallFrame pushcall() {
      if (calls >= frame.length) {
        ...
        for (int i = frame.length; i < n; ++i) f[i] = new CallFrame();
        frame = f;
        for (int i = 1; i < n; ++i) f[i].previous = f[i - 1];
      }
      return frame[calls++];
    }
    ...
    synchronized String traceback(int level) {
      ...
      for (DebugLib.CallFrame c; (c = getCallFrame(level++)) != null; ) {
        ...
        sb.append(':');
        if (c.currentline() > 0) sb.append(c.currentline() + ":");
        sb.append(" in ");
    ...
    synchronized DebugLib.CallFrame findCallFrame(LuaValue func) {
      for (int i = 1; i <= calls; ++i) if (frame[calls - i].f == func) return frame[i];
      return null;
    }
  ...
  public static class CallFrame {
    ...
    void instr(int pc, Varargs v, int top) {
      ...
      this.v = v;
      this.top = top;
      if (TRACE) Print.printState(f.checkclosure(), pc, stack, top, v);
    }
  ...
  static NameWhat getfuncname(DebugLib.CallFrame frame) {
    if (!frame.f.isclosure()) return new NameWhat(frame.f.classnamestub(), "Java");
  ...
  public static NameWhat getobjname(Prototype p, int lastpc, int reg) {
    ...
    if (name != null) /* is a local? */ return new NameWhat(name.tojstring(), "local");
        ...
        case Lua.OP_MOVE:
          {
            ...
            if (b < a) return getobjname(p, pc, b); /* get name for `b' */
            break;
          }
        case Lua.OP_GETTABUP:
        case Lua.OP_GETTABLE:
          {
            ...
            LuaString vn =
                (Lua.GET_OPCODE(i) == Lua.OP_GETTABLE) /* name of indexed variable */
  ...
  static int findsetreg(Prototype p, int lastpc, int reg) {
    ...
    for (pc = 0; pc < lastpc; pc++) {
      ...
      switch (op) {
        case Lua.OP_LOADNIL:
          {
            ...
            if (a <= reg && reg <= a + b) /* set registers from 'a' to 'a+b' */ setreg = pc;
            break;
          }
        ...
        default:
          if (Lua.testAMode(op) && reg == a) /* any instruction that set A */ setreg = pc;
          break;
      }
    }
    return setreg;
