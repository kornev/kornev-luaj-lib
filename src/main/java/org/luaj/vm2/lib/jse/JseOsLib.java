 ...
 *
 * <p>This contains more complete implementations of the following functions using features that are
 * specific to JSE:
 *
 * <ul>
 *   <li>{@code execute()}
...
public class JseOsLib extends org.luaj.vm2.lib.OsLib {
  ...
  protected Varargs execute(String command) {
    ...
    if (exitValue == 0) return varargsOf(TRUE, valueOf("exit"), ZERO);
    return varargsOf(NIL, valueOf("signal"), valueOf(exitValue));
  }

  protected void remove(String filename) throws IOException {
    File f = new File(filename);
    if (!f.exists()) throw new IOException("No such file or directory");
  ...
  protected void rename(String oldname, String newname) throws IOException {
    ...
    if (!f.exists()) throw new IOException("No such file or directory");
    if (!f.renameTo(new File(newname))) throw new IOException("Failed to rename");
  }
