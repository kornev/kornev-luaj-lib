 ...
 *
 * <p>Luajava is an approach to mixing lua and java using simple functions that bind java classes
 * and methods to lua dynamically. The API is documented on the <a
 * href="http://www.keplerproject.org/luajava/">luajava</a> documentation pages.
 *
 ...
 * }</pre>
 *
 * <p>To instantiate and use it directly, link it into your globals table via {@link Globals#load}
 * using code such as:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The {@code luajava} library is available on all JSE platforms via the call to {@link
 * org.luaj.vm2.lib.jse.JsePlatform#standardGlobals()} and the luajava api's are simply invoked from
 * lua. Because it makes extensive use of Java's reflection API, it is not available on JME, but can
...
public class LuajavaLib extends VarArgFunction {
  ...
  static final String[] NAMES = {
    "bindClass", "newInstance", "new", "createProxy", "loadLib",
  };
        ...
        case CREATEPROXY:
          {
            ...
            for (int i = 0; i < niface; i++) ifaces[i] = classForName(args.checkjstring(i + 1));
  ...
  private static final class ProxyInvocationHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (isvarargs) {
        ...
        v = new LuaValue[n + m];
        for (int i = 0; i < n; i++) v[i] = CoerceJavaToLua.coerce(args[i]);
        for (int i = 0; i < m; i++) v[i + n] = CoerceJavaToLua.coerce(Array.get(o, i));
      } else {
        v = new LuaValue[n];
        for (int i = 0; i < n; i++) v[i] = CoerceJavaToLua.coerce(args[i]);
      }
