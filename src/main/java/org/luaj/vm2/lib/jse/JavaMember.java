 ...
 *
 * <p>Primarily handles argument coercion for parameter lists including scoring of compatibility and
 * java varargs handling.
 *
 * <p>This class is not used directly. It is an abstract base class for {@link JavaConstructor} and
 * {@link JavaMethod}.
 *
 * @see JavaConstructor
 * @see JavaMethod
...
abstract class JavaMember extends VarArgFunction {
  ...
  int score(Varargs args) {
    ...
    for (int j = 0; j < fixedargs.length; j++) s += fixedargs[j].score(args.arg(j + 1));
    if (varargs != null)
      for (int k = fixedargs.length; k < n; k++) s += varargs.score(args.arg(k + 1));
    return s;
  }
  ...
  protected Object[] convertArgs(Varargs args) {
    ...
    if (varargs == null) {
      ...
      for (int i = 0; i < a.length; i++) a[i] = fixedargs[i].coerce(args.arg(i + 1));
    } else {
      int n = Math.max(fixedargs.length, args.narg());
      a = new Object[n];
      for (int i = 0; i < fixedargs.length; i++) a[i] = fixedargs[i].coerce(args.arg(i + 1));
      for (int i = fixedargs.length; i < n; i++) a[i] = varargs.coerce(args.arg(i + 1));
    }
    return a;
  }
}
