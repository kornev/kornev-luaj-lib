...
public class JseProcess {
   ...
   * @param stdin Optional InputStream to read from as process input, or null if input is not
   *     needed.
   * @param stdout Optional OutputStream to copy process output to, or null if output is ignored.
   * @param stderr Optinoal OutputStream to copy process stderr output to, or null if output is
   *     ignored.
   * @throws IOException If the system process could not be created.
   * @see Process
   */
  public JseProcess(String[] cmd, InputStream stdin, OutputStream stdout, OutputStream stderr)
      throws IOException {
  ...
  private JseProcess(Process process, InputStream stdin, OutputStream stdout, OutputStream stderr) {
    ...
    input =
        stdin == null
            ? null
            : copyBytes(stdin, process.getOutputStream(), null, process.getOutputStream());
    output =
        stdout == null
            ? null
            : copyBytes(process.getInputStream(), stdout, process.getInputStream(), null);
    error =
        stderr == null
            ? null
            : copyBytes(process.getErrorStream(), stderr, process.getErrorStream(), null);
  }
  ...
  private static final class CopyThread extends Thread {
    ...
    private CopyThread(
        OutputStream output, OutputStream ownedOutput, InputStream ownedInput, InputStream input) {
      this.output = output;
