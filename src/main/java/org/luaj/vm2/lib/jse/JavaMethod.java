 ...
 *
 * <p>Can be invoked via call(LuaValue...) and related methods.
 *
 * <p>This class is not used directly. It is returned by calls to calls to {@link
 * JavaInstance#get(LuaValue key)} when a method is named.
 *
 * @see CoerceJavaToLua
 * @see CoerceLuaToJava
 */
class JavaMethod extends JavaMember {
  ...
  static JavaMethod forMethod(Method m) {
    ...
    if (j == null) methods.put(m, j = new JavaMethod(m));
    return j;
  }
   ...
   *
   * <p>This class is not used directly. It is returned by calls to calls to {@link
   * JavaInstance#get(LuaValue key)} when an overloaded method is named.
   */
