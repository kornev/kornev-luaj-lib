 ...
 *
 * <p>Can get elements by their integer key index, as well as the length.
 *
 * <p>This class is not used directly. It is returned by calls to {@link
 * CoerceJavaToLua#coerce(Object)} when an array is supplied.
 *
 * @see CoerceJavaToLua
 * @see CoerceLuaToJava
 */
class JavaArray extends LuaUserdata {
  ...
  public LuaValue get(LuaValue key) {
    if (key.equals(LENGTH)) return valueOf(Array.getLength(m_instance));
    if (key.isint()) {
      int i = key.toint() - 1;
  ...
  public void set(LuaValue key, LuaValue value) {
    if (key.isint()) {
      ...
      if (i >= 0 && i < Array.getLength(m_instance))
        Array.set(
            m_instance, i, CoerceLuaToJava.coerce(value, m_instance.getClass().getComponentType()));
