 ...
 *
 * <p>It contains only the math library support that is possible on JME. For a more complete
 * implementation based on math functions specific to JSE use {@link
 * org.luaj.vm2.lib.jse.JseMathLib}. In Particular the following math functions are <b>not</b>
 ...
 * </ul>
 *
 * <p>The implementations of {@code exp()} and {@code pow()} are constructed by hand for JME, so
 * will be slower and less accurate than when executed on the JSE platform.
 *
 ...
 * }</pre>
 *
 * When using {@link org.luaj.vm2.lib.jse.JsePlatform} as in this example, the subclass {@link
 * org.luaj.vm2.lib.jse.JseMathLib} will be included, which also includes this base functionality.
 *
...
public class MathLib extends TwoArgFunction {
  ...
  /**
   * Pointer to the latest MathLib instance, used only to dispatch math.exp to tha correct platform
   * math library.
   */
  public static MathLib MATHLIB = null;

  /**
   * Construct a MathLib, which can be initialized by calling it with a modname string, and a global
   * environment table as arguments using {@link #call(LuaValue, LuaValue)}.
   */
  public MathLib() {
    MATHLIB = this;
  }
  ...
  protected abstract static class BinaryOp extends TwoArgFunction {
    ...
    protected abstract double call(double x, double y);
  }
  ...
  protected static double dpow_default(double a, double b) {
    if (b < 0) return 1 / dpow_default(a, -b);
    double p = 1;
    int whole = (int) b;
