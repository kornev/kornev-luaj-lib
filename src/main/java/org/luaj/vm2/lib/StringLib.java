 ...
 * }</pre>
 *
 * <p>To instantiate and use it directly, link it into your globals table via {@link
 * LuaValue#load(LuaValue)} using code such as:
 *
 * <pre>{@code
...
public class StringLib extends TwoArgFunction {
  ...
  /**
   * Construct a StringLib, which can be initialized by calling it with a modname string, and a
   * global environment table as arguments using {@link #call(LuaValue, LuaValue)}.
   */
  public StringLib() {}
   ...
   * string.byte (s [, i [, j]])
   *
   * <p>Returns the internal numerical codes of the characters s[i], s[i+1], ..., s[j]. The default
  ...
  static final class _byte extends VarArgFunction {
    public Varargs invoke(Varargs args) {
      ...
      n = (int) (pose - posi + 1);
      if (posi + n <= pose) /* overflow? */ error("string slice too long");
   ...
   * string.char (...)
   *
   * <p>Receives zero or more integers. Returns a string with length equal to the number of
   * arguments, in which each character has the internal numerical code equal to its corresponding
   * argument.
   *
   * <p>Note that numerical codes are not necessarily portable across platforms.
   *
   ...
   *
   * <p>Returns a string containing a binary representation of the given function, so that a later
   * loadstring on this string returns a copy of the function. function must be a Lua function
   * without upvalues. Boolean param stripDebug - true to strip debugging info, false otherwise. The
   * default value for stripDebug is true.
   *
   * <p>TODO: port dumping code as optional add-on
   */
   ...
   *
   * <p>Looks for the first match of pattern in the string s. If it finds a match, then find returns
   ...
   * and may be negative. A value of true as a fourth, optional argument plain turns off the pattern
   * matching facilities, so the function does a plain "find substring" operation, with no
   * characters in pattern being considered "magic". Note that if plain is given, then init must be
   * given as well.
   *
   * <p>If the pattern has captures, then in a successful match the captured values are also
   ...
   *
   * <p>Returns a formatted version of its variable number of arguments following the description
   * given in its first argument (which must be a string). The format string follows the same rules
   * as the printf family of standard C functions. The only differences are that the
   * options/modifiers *, l, L, n, p, and h are not supported and that there is an extra option, q.
   ...
   * and backslashes in the string are correctly escaped when written. For instance, the call
   * string.format('%q', 'a string with "quotes" and \n new line')
   *
   * <p>will produce the string: "a string with \"quotes\" and \ new line"
   *
   * <p>The options c, d, E, e, f, g, G, i, o, u, X, and x all expect a number as argument, whereas
   * q and s expect a string.
   *
   * <p>This function does not accept string values containing embedded zeros, except as arguments
   * to the q option.
   */
   ...
   *
   * <p>Returns an iterator function that, each time it is called, returns the next captures from
   * pattern over string s. If pattern specifies no captures, then the whole match is produced in
   * each call.
   *
   * <p>As an example, the following loop s = "hello world from Lua" for w in string.gmatch(s,
   * "%a+") do print(w) end
   *
   * <p>will iterate over all the words from string s, printing one per line. The next example
   * collects all pairs key=value from the given string into a table: t = {} s = "from=world,
   * to=Lua" for k, v in string.gmatch(s, "(%w+)=(%w+)") do t[k] = v end
   *
   * <p>For this function, a '^' at the start of a pattern does not work as an anchor, as this would
  ...
  /**
   * string.gsub (s, pattern, repl [, n]) Returns a copy of s in which all (or the first n, if
   * given) occurrences of the pattern have been replaced by a replacement string specified by repl,
   * which may be a string, a table, or a function. gsub also returns, as its second value, the
   * total number of matches that occurred.
   *
   * <p>If repl is a string, then its value is used for replacement. The character % works as an
   * escape character: any sequence in repl of the form %n, with n between 1 and 9, stands for the
   * value of the n-th captured substring (see below). The sequence %0 stands for the whole match.
   * The sequence %% stands for a single %.
   *
   * <p>If repl is a table, then the table is queried for every match, using the first capture as
   * the key; if the pattern specifies no captures, then the whole match is used as the key.
   *
   * <p>If repl is a function, then this function is called every time a match occurs, with all
   * captured substrings passed as arguments, in order; if the pattern specifies no captures, then
   * the whole match is passed as a sole argument.
   *
   * <p>If the value returned by the table query or by the function call is a string or a number,
   * then it is used as the replacement string; otherwise, if it is false or nil, then there is no
   * replacement (that is, the original match is kept in the string).
   *
   * <p>Here are some examples: x = string.gsub("hello world", "(%w+)", "%1 %1") --> x="hello hello
   * world world"
   *
   * <p>x = string.gsub("hello world", "%w+", "%0 %0", 1) --> x="hello hello world"
   *
   * <p>x = string.gsub("hello world from Lua", "(%w+)%s*(%w+)", "%2 %1") --> x="world hello Lua
   * from"
   *
   * <p>x = string.gsub("home = $HOME, user = $USER", "%$(%w+)", os.getenv) --> x="home =
   * /home/roberto, user = roberto"
   *
   * <p>x = string.gsub("4+5 = $return 4+5$", "%$(.-)%$", function (s) return loadstring(s)() end)
   * --> x="4+5 = 9"
   *
   * <p>local t = {name="lua", version="5.1"} x = string.gsub("$name-$version.tar.gz", "%$(%w+)", t)
   * --> x="lua-5.1.tar.gz"
   */
   ...
   * string.len (s)
   *
   * <p>Receives a string and returns its length. The empty string "" has length 0. Embedded zeros
   ...
   * string.lower (s)
   *
   * <p>Receives a string and returns a copy of this string with all uppercase letters changed to
   * lowercase. All other characters are left unchanged. The definition of what an uppercase letter
   ...
   *
   * <p>Looks for the first match of pattern in the string s. If it finds one, then match returns
   * the captures from the pattern; otherwise it returns nil. If pattern specifies no captures, then
   * the whole match is returned. A third, optional numerical argument init specifies where to start
   ...
   * string.rep (s, n)
   *
   * <p>Returns a string that is the concatenation of n copies of the string s.
   */
   ...
   * string.reverse (s)
   *
   * <p>Returns a string that is the string s reversed.
   */
   ...
   * string.sub (s, i [, j])
   *
   * <p>Returns the substring of s that starts at i and continues until j; i and j may be negative.
   * If j is absent, then it is assumed to be equal to -1 (which is the same as the string length).
   * In particular, the call string.sub(s,1,j) returns a prefix of s with length j, and
   ...
   * string.upper (s)
   *
   * <p>Receives a string and returns a copy of this string with all lowercase letters changed to
  ...
  static {
    ...
    for (int i = 0; i < 128; ++i) {
      ...
      CHAR_TABLE[i] =
          (byte)
              ((Character.isDigit(c) ? MASK_DIGIT : 0)
  ...
  static class MatchState {
          ...
          if (!Character.isDigit((char) b)) {
            if (b != L_ESC)
              error(
                  "invalid use of '"
                      + (char) L_ESC
                      + "' in replacement string: after '"
                      + (char) L_ESC
                      + "' must be '0'-'9' or '"
                      + (char) L_ESC
                      + "', but found "
                      + (i < l
                          ? "symbol '" + (char) b + "' with code " + b + " at pos " + (i + 1)
                          : "end of string"));
            lbuf.append(b);
          } else if (b == '0') {
    ...
    Varargs push_captures(boolean wholeMatch, int soff, int end) {
      ...
      for (int i = 0; i < nlevels; ++i) v[i] = push_onecapture(i, soff, end);
      return varargsOf(v);
    }
    ...
    private int capture_to_close() {
      ...
      for (level--; level >= 0; level--) if (clen[level] == CAP_UNFINISHED) return level;
    ...
    /**
     * Perform pattern matching. If there is a match, returns offset into s where match ends,
     * otherwise returns -1.
     */
                ...
                case 'f':
                  {
                    ...
                    if (matchbracketclass(previous, poffset, ep - 1)
                        || !matchbracketclass(next, poffset, ep - 1)) return -1;
                    poffset = ep;
                    continue;
                  }
                default:
    ...
    int max_expand(int soff, int poff, int ep) {
      int i = 0;
      while (soff + i < s.length() && singlematch(s.luaByte(soff + i), poff, ep)) i++;
      while (i >= 0) {
