...
/**
 * Abstract base class extending {@link LibFunction} which implements the core of the lua standard
 * {@code io} library.
 *
 * <p>It contains the implementation of the io library support that is common to the JSE and JME
 * platforms. In practice on of the concrete IOLib subclasses is chosen: {@link
 * org.luaj.vm2.lib.jse.JseIoLib} for the JSE platform.
 *
 * <p>The JSE implementation conforms almost completely to the C-based lua library, while the JME
 * implementation follows closely except in the area of random-access files, which are difficult to
 * support properly on JME.
 *
 ...
 * }</pre>
 *
 * In this example the platform-specific {@link org.luaj.vm2.lib.jse.JseIoLib} library will be
 * loaded, which will include the base functionality provided by this class, whereas the {@link
 * org.luaj.vm2.lib.jse.JsePlatform} would load the {@link org.luaj.vm2.lib.jse.JseIoLib}.
 *
 ...
 * }</pre>
 *
 * <p>This has been implemented to match as closely as possible the behavior in the corresponding
 * library in C.
 *
 * @see LibFunction
...
public abstract class IoLib extends TwoArgFunction {
  ...
  protected abstract File openFile(
      String filename, boolean readMode, boolean appendMode, boolean updateMode, boolean binaryMode)
      throws IOException;
  ...
  private void setLibInstance(LuaTable t) {
    LuaValue[] k = t.keys();
    for (int i = 0, n = k.length; i < n; i++) ((IoLibV) t.get(k[i])).iolib = this;
  }
  ...
  public Varargs _io_index(LuaValue v) {
    return v.equals(STDOUT)
        ? output()
        : v.equals(STDIN) ? input() : v.equals(STDERR) ? errput() : NIL;
  }
  ...
  private static Varargs iowrite(File f, Varargs args) throws IOException {
    for (int i = 1, n = args.narg(); i <= n; i++) f.write(args.checkstring(i));
    return f;
  }
  ...
  private static void freadchars(File f, String chars, ByteArrayOutputStream baos)
      throws IOException {
    int c;
    while (true) {
      c = f.peek();
