 ...
 *
 * <p>It is a usable base with simplified stub functions for library functions that cannot be
 * implemented uniformly on Jse and Jme.
 *
 * <p>This can be installed as-is on either platform, or extended and refined to be used in a
 * complete Jse implementation.
 *
 * <p>Because the nature of the {@code os} library is to encapsulate os-specific features, the
 * behavior of these functions varies considerably from their counterparts in the C platform.
 *
 * <p>The following functions have limited implementations of features that are not supported well
 * on Jme:
 *
 * <ul>
 *   <li>{@code execute()}
 ...
 * }</pre>
 *
 * In this example the platform-specific {@link org.luaj.vm2.lib.jse.JseOsLib} library will be
 * loaded, which will include the base functionality provided by this class.
 *
...
public class OsLib extends TwoArgFunction {
  ...
  /**
   * @return an approximation of the amount in seconds of CPU time used by the program. For luaj
   *     this simple returns the elapsed time since the OsLib class was loaded.
   */
   ...
   *
   * <p>Date returns the date as a string, formatted according to the same rules as ANSII strftime,
   * but without support for %g, %G, or %V.
   *
   * <p>When called without arguments, date returns a reasonable date and time representation that
   ...
   * @return a LString or a LTable containing date and time, formatted according to the given string
   *     format.
   */
          ...
          switch (c = fmt[i++]) {
            ...
            case 'j':
              { // day of year.
                ...
                int dayOfYear =
                    (int) ((d.getTime().getTime() - y0.getTime().getTime()) / (24 * 3600L * 1000L));
  ...
  /**
   * Returns the value of the process environment variable varname, or the System property value for
   * varname, or null if the variable is not defined in either environment.
   *
   * <p>The default implementation, which is used by the JmePlatform, only queryies
   * System.getProperty().
   *
   * <p>The JsePlatform overrides this behavior and returns the environment variable value using
   * System.getenv() if it exists, or the System property value if it does not.
   *
   * <p>A SecurityException may be thrown if access is not allowed for 'varname'.
   *
   * @param varname
   ...
   * this function fails, it throws and IOException
   *
   * @param filename
  ...
  /**
   * Renames file or directory named oldname to newname. If this function fails,it throws and
   * IOException
   *
   ...
   * "numeric", or "time"; the default category is "all".
   *
   * <p>If locale is the empty string, the current locale is set to an implementation- defined
   * native locale. If locale is the string "C", the current locale is set to the standard C locale.
   *
   * <p>When called with null as the first argument, this function only returns the name of the
   ...
   * have fields hour, min, sec, and isdst (for a description of these fields, see the os.date
   * function).
   *
   * @param table
   ...
   *
   * <p>On some systems (POSIX), this function also creates a file with that name, to avoid security
