...
public class LexState extends Constants {
  ...
  static final Hashtable RESERVED = new Hashtable();
  ...
  void save(int c) {
    if (buff == null || nbuff + 1 > buff.length) buff = realloc(buff, nbuff * 2 + 1);
    buff[nbuff++] = (char) c;
  }
  ...
  private void skipShebang() {
    if (current == '#') while (!currIsNewline() && current != EOZ) nextChar();
  }
  ...
  LuaValue strx2number(String str, SemInfo seminfo) {
    ...
    int s = 0;
    while (s < c.length && isspace(c[s])) ++s;
    ...
    /* Check for "0x" */
    if (s + 2 >= c.length) return LuaValue.ZERO;
    ...
    double m = 0;
    int e = 0;
    while (s < c.length && isxdigit(c[s])) m = (m * 16) + hexvalue(c[s++]);
    if (s < c.length && c[s] == '.') {
      ++s; // skip dot
    ...
    if (s < c.length && (c[s] == 'p' || c[s] == 'P')) {
      ...
      while (s < c.length && isdigit(c[s])) exp1 = exp1 * 10 + c[s++] - '0';
      if (neg1) exp1 = -exp1;
      e += exp1;
    }
  ...
  void read_numeral(SemInfo seminfo) {
    ...
    save_and_next();
    if (first == '0' && check_next("Xx")) expo = "Pp";
    while (true) {
      if (check_next(expo)) check_next("+-");
      if (isxdigit(current) || current == '.') save_and_next();
      else break;
    }
  ...
  void read_long_string(SemInfo seminfo, int sep) {
    ...
    if (currIsNewline()) /* string starts with a newline? */ inclinenumber(); /* skip it */
    for (boolean endloop = false; !endloop; ) {
      switch (current) {
        case EOZ:
          lexerror(
              (seminfo != null) ? "unfinished long string" : "unfinished long comment", TK_EOS);
        ...
        case '-':
          {
            ...
            /* else short comment */
            while (!currIsNewline() && current != EOZ) nextChar();
            continue;
          }
        case '[':
          {
        ...
        default:
          {
            if (isalpha(current) || current == '_') {
              ...
              if (RESERVED.containsKey(ts)) return ((Integer) RESERVED.get(ts)).intValue();
              else {
                seminfo.ts = ts;
                return TK_NAME;
              }
  ...
  void undefgoto(Labeldesc gt) {
    String msg =
        L.pushfstring(
            isReservedKeyword(gt.name.tojstring())
        ...
        case TK_NAME:
          {
            ...
            this.lookahead();
            if (this.lookahead.token != '=') /* expression? */ this.listfield(cc);
      ...
      case '(':
        {
          /* funcargs -> `(' [ explist1 ] `)' */
          this.next();
          if (this.t.token == ')') /* arg list is empty? */ args.k = VVOID;
          else {
            this.explist(args);
            fs.setmultret(args);
          }
      ...
      case TK_DOTS:
        {
          ...
          this.check_condition(
              fs.f.is_vararg != 0, "cannot use " + LUA_QL("...") + " outside a vararg function");
  ...
  static Priority[] priority = {
    ...
    new Priority(7, 7), /* `+' `-' `/' `%' */
    new Priority(10, 9),
    new Priority(5, 4), /* power and concat (right associative) */
    new Priority(3, 3),
    new Priority(3, 3), /* equality and inequality */
    new Priority(3, 3),
    new Priority(3, 3),
    new Priority(3, 3),
    new Priority(3, 3), /* order */
    new Priority(2, 2),
    new Priority(1, 1) /* logical (and/or) */
  };
  ...
  void assignment(LHS_assign lh, int nvars) {
    ...
    if (this.testnext(',')) {
      ...
      if (nv.v.k != VINDEXED) this.check_conflict(lh, nv.v);
  ...
  void gotostat(int pc) {
    ...
    LuaString label;
    int g;
    if (testnext(TK_GOTO)) label = str_checkname();
    else {
      next(); /* skip break */
  ...
  void skipnoopstat() {
    while (t.token == ';' || t.token == TK_DBCOLON) statement();
  }
  ...
  void labelstat(LuaString label, int line) {
    ...
    l =
        newlabelentry(
            dyd.label = grow(dyd.label, dyd.n_label + 1),
            dyd.n_label++,
            label,
            line,
            fs.getlabel());
  ...
  void forbody(int base, int line, int nvars, boolean isnum) {
    ...
    fs.patchtohere(prep);
    if (isnum) /* numeric for? */ endfor = fs.codeAsBx(Lua.OP_FORLOOP, base, NO_JUMP);
    else {
      /* generic for */
  ...
  void ifstat(int line) {
    ...
    while (t.token == TK_ELSEIF) test_then_block(escapelist); /* ELSEIF cond THEN block */
  ...
  void localstat() {
    ...
    if (this.testnext('=')) nexps = this.explist(e);
    else {
      e.k = VVOID;
      nexps = 0;
    }
  ...
  boolean funcname(expdesc v) {
    ...
    this.singlevar(v);
    while (this.t.token == '.') this.fieldsel(v);
    if (this.t.token == ':') {
      ismethod = true;
      this.fieldsel(v);
    }
  ...
  void retstat() {
    ...
    if (block_follow(true) || this.t.token == ';') first = nret = 0; /* return no values */
    else {
      ...
      if (hasmultret(e.k)) {
        ...
        if (nret == 1) /* only one single value? */ first = fs.exp2anyreg(e);
        else {
      ...
      case TK_LOCAL:
        {
          ...
          if (this.testnext(TK_FUNCTION)) /* local function? */ this.localfunc();
          else this.localstat();
          break;
        }
      case TK_DBCOLON:
