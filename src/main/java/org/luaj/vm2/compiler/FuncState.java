...
public class FuncState extends Constants {
  ...
  void removevars(int tolevel) {
    ...
    while (nactvar > tolevel) getlocvar(--nactvar).endpc = pc;
  }

  int searchupvalue(LuaString name) {
    int i;
    Upvaldesc[] up = f.upvalues;
    for (i = 0; i < nups; i++) if (up[i].name.eq_b(name)) return i;
    return -1; /* not found */
  }
  ...
  void movegotosout(BlockCnt bl) {
    ...
    while (i < ls.dyd.n_gt) {
      ...
      if (gt.nactvar > bl.nactvar) {
        if (bl.upval) patchclose(gt.pc, bl.nactvar);
  ...
  void nil(int from, int n) {
    ...
    if (this.pc > this.lasttarget && pc > 0) {
  ...
  void removevalues(int list) {
    for (; list != LexState.NO_JUMP; list = this.getjump(list)) this.patchtestreg(list, NO_REG);
  }
  ...
  int addk(LuaValue v) {
    ...
    if (f.k == null || nk + 1 >= f.k.length) f.k = realloc(f.k, nk * 2 + 1);
    f.k[this.nk++] = v;
    return idx;
  }
