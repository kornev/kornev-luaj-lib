 ...
 *
 * <p>Generally, this class is not used directly, but rather indirectly via a command line interface
 * tool such as {@link luac}.
 *
 * <p>A lua binary file is created via {@link DumpState#dump}:
 *
 * <pre>{@code
...
public class DumpState {
  ...
  void dumpCode(final Prototype f) throws IOException {
    ...
    int n = code.length;
    dumpInt(n);
    for (int i = 0; i < n; i++) dumpInt(code[i]);
  }
  ...
  public static int dump(
      Prototype f, OutputStream w, boolean stripDebug, int numberFormat, boolean littleendian)
      throws IOException {
    switch (numberFormat) {
      case NUMBER_FORMAT_FLOATS_OR_DOUBLES:
      case NUMBER_FORMAT_INTS_ONLY:
