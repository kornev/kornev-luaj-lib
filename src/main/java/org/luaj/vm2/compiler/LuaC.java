 ...
 * Compiler for Lua.
 *
 * <p>Compiles lua source files into lua bytecode within a {@link Prototype}, loads lua binary files
 * directly into a {@link Prototype}, and optionaly instantiates a {@link LuaClosure} around the
 * result using a user-supplied environment.
 *
 * <p>Implements the {@link org.luaj.vm2.Globals.Compiler} interface for loading initialized chunks,
 * which is an interface common to lua bytecode compiling and java bytecode compiling.
 *
 * <p>The {@link LuaC} compiler is installed by default the {@link org.luaj.vm2.lib.jse.JsePlatform}
 * class, so in the following example, the default {@link LuaC} compiler will be used:
 *
 * <pre>{@code
...
public class LuaC extends Constants implements Globals.Compiler, Globals.Loader {
  ...
  /**
   * Install the compiler so that LoadState will first try to use it when handed bytes that are not
   * already a compiled lua chunk.
   *
  ...
  /**
   * @deprecated Use Globals.load(InputString, String, String) instead, or LuaC.compile(InputStream,
   *     String) and construct LuaClosure directly.
   */
  @Deprecated
  ...
  static class CompileState {
    ...
    Prototype luaY_parser(InputStream z, String name) throws IOException {
      ...
      LuaC._assert(
          lexstate.dyd == null
              || (lexstate.dyd.n_actvar == 0
                  && lexstate.dyd.n_gt == 0
                  && lexstate.dyd.n_label == 0));
      return funcstate.f;
    }
