 ...
 *
 * <p>This engine requires the types of the Bindings and ScriptContext to be compatible with the
 * engine. For creating new client context use ScriptEngine.createContext() which will return {@link
 * LuajContext}, and for client bindings use the default engine scoped bindings or construct a
...
public class LuaScriptEngine extends AbstractScriptEngine implements ScriptEngine, Compilable {
  ...
  class LuajCompiledScript extends CompiledScript {
    ...
    Object eval(Globals g, Bindings b) throws ScriptException {
      ...
      if (f.isclosure()) f = new LuaClosure(f.checkclosure().p, g);
      else {
        try {
