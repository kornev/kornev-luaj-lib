 ...
 *
 * <p>The buffer can begin initially as a wrapped {@link LuaValue} and only when concatenation
 * actually occurs are the bytes first copied.
 *
 * <p>To convert back to a {@link LuaValue} again, the function {@link Buffer#value()} is used.
 *
 * @see LuaValue
 * @see LuaValue#buffer()
 * @see LuaString
...
public final class Buffer {
  ...
  /**
   * Append a Java String to the buffer. The Java string will be converted to bytes using the UTF8
   * encoding.
   *
   * @return {@code this} to allow call chaining
   ...
   *
   * <p>The {@link LuaNumber} will be converted to a string before concatenating.
   *
   * @param lhs the left-hand-side value onto which we are concatenating {@code this}
   * @return {@link Buffer} for use in call chaining.
   */
  public Buffer concatTo(LuaNumber lhs) {
    return value != null && !value.isstring()
        ? setvalue(lhs.concat(value))
        : prepend(lhs.strvalue());
  }
