 ...
 *
 * <p>The {@link LoadState} class provides the default {@link Globals.Undumper} which is used to
 * undump a string of bytes that represent a lua binary file using either the C-based lua compiler,
 * or luaj's {@link org.luaj.vm2.compiler.LuaC} compiler.
 *
 * <p>The canonical method to load and execute code is done indirectly using the Globals:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * This should work regardless of which {@link Globals.Compiler} or {@link Globals.Undumper} have
 * been installed.
 *
 * <p>By default, when using {@link org.luaj.vm2.lib.jse.JsePlatform} to construct globals, the
 * {@link LoadState} default undumper is installed as the default {@link Globals.Undumper}.
 *
 * <p>A lua binary file is created via the {@link org.luaj.vm2.compiler.DumpState} class :
 *
 * <pre>{@code
 * Globals globals = JsePlatform.standardGlobals();
 ...
 * }</pre>
 *
 * The {@link LoadState}'s default undumper {@link #instance} may be used directly to undump these
 * bytes:
 *
 * <pre>{@code
...
public class LoadState {
  ...
  void loadDebug(Prototype f) throws IOException {
    ...
    n = loadInt();
    for (int i = 0; i < n; i++) f.upvalues[i].name = loadString();
  }
   ...
   * @return {@link Prototype} that was loaded, or null if the first 4 bytes were not the lua
   *     signature.
  ...
  public static Prototype undump(InputStream stream, String chunkname) throws IOException {
    ...
    if (stream.read() != LUA_SIGNATURE[0]
        ...
        || stream.read() != LUA_SIGNATURE[3]) return null;
