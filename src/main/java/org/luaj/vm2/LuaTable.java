 ...
 *
 * <p>Almost all API's implemented in {@link LuaTable} are defined and documented in {@link
 * LuaValue}.
 *
 * <p>If a table is needed, the one of the type-checking functions can be used such as {@link
 * #istable()}, {@link #checktable()}, or {@link #opttable(LuaTable)}
 *
 * <p>The main table operations are defined on {@link LuaValue} for getting and setting values with
 * and without metatag processing:
 *
 * <ul>
 *   <li>{@link #get(LuaValue)}
 *   <li>{@link #set(LuaValue,LuaValue)}
 *   <li>{@link #rawget(LuaValue)}
 *   <li>{@link #rawset(LuaValue,LuaValue)}
 *   <li>plus overloads such as {@link #get(String)}, {@link #get(int)}, and so on
 * </ul>
 *
 * <p>To iterate over key-value pairs from Java, use
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>As with other types, {@link LuaTable} instances should be constructed via one of the table
 * constructor methods on {@link LuaValue}:
 *
 * <ul>
 *   <li>{@link LuaValue#tableOf()} empty table
 *   <li>{@link LuaValue#tableOf(int, int)} table with capacity
 *   <li>{@link LuaValue#listOf(LuaValue[])} initialize array part
 *   <li>{@link LuaValue#listOf(LuaValue[], Varargs)} initialize array part
 *   <li>{@link LuaValue#tableOf(LuaValue[])} initialize named hash part
 *   <li>{@link LuaValue#tableOf(Varargs, int)} initialize named hash part
 *   <li>{@link LuaValue#tableOf(LuaValue[], LuaValue[])} initialize array and named parts
 *   <li>{@link LuaValue#tableOf(LuaValue[], LuaValue[], Varargs)} initialize array and named parts
 * </ul>
 *
 * @see LuaValue
 */
public class LuaTable extends LuaValue implements Metatable {
  ...
  public LuaTable(LuaValue[] named, LuaValue[] unnamed, Varargs lastarg) {
    ...
    presize(nu + nl, nn >> 1);
    for (int i = 0; i < nu; i++) rawset(i + 1, unnamed[i]);
    if (lastarg != null)
      for (int i = 1, n = lastarg.narg(); i <= n; ++i) rawset(nu + i, lastarg.arg(i));
    for (int i = 0; i < nn; i += 2) if (!named[i + 1].isnil()) rawset(named[i], named[i + 1]);
  }
  ...
  public void presize(int narray) {
    if (narray > array.length) array = resize(array, 1 << log2(narray));
  }
  ...
  public Varargs next(LuaValue key) {
    ...
    do {
      ...
      if (!key.isnil()) {
        ...
        if (hash.length == 0) error("invalid key to 'next' 1: " + key);
        i = hashSlot(key);
        boolean found = false;
  ...
  public void sort(LuaValue comparator) {
    ...
    int n = length();
    if (n > 1) heapSort(n, comparator.isnil() ? null : comparator);
  }
  ...
  private void heapify(int count, LuaValue cmpfunc) {
    for (int start = count / 2; start > 0; --start) siftDown(start, count, cmpfunc);
  }
  ...
  /**
   * This may be deprecated in a future release. It is recommended to count via iteration over
   * next() instead
   *
  ...
  public Varargs unpack(int i, int j) {
    ...
    if (count < 0)
      throw new LuaError(
          "too many results to unpack: greater " + Integer.MAX_VALUE); // integer overflow
    int max = 0x00ffffff;
    if (count >= max)
  ...
  interface Slot {
    ...
    /**
     * Compare given key with first()'s key; return true if equal. May return true for keys no
     * longer present in the table.
     */
    ...
    /**
     * Return first entry's key, iff it is an integer between 1 and max, inclusive, or zero
     * otherwise.
     */
    int arraykey(int max);

    /**
     * Set the value of this Slot's first Entry, if possible, or return a new Slot whose first entry
     * has the given value.
     */
    ...
    /**
     * Return a Slot with the given value set to nil; must not return null for next() to behave
     * correctly.
     */
  ...
  /**
   * Subclass of Slot guaranteed to have a strongly-referenced key and value, to support weak
   * tables.
   */
   ...
   *
   * <p>If the key may be an integer, the {@link #arraykey(int)} method must be overridden to handle
   * that case.
   */
  ...
  /**
   * A Slot whose value has been set to nil. The key is kept in a weak reference so that it can be
   * found by next().
   */
