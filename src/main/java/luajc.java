...
public class luajc {
  ...
  private luajc(String[] args) throws IOException {
    ...
    for (int i = 0; i < seeds.size(); i++) collectFiles(srcdir + "/" + seeds.get(i));
    ...
    for (int i = 0, n = files.size(); i < n; i++) processFile((InputFile) files.get(i));
  }
  ...
  private void scandir(File dir, String javapackage) {
    ...
    for (int i = 0; i < f.length; i++) scanfile(dir, f[i], javapackage);
  }
  ...
  private void processFile(InputFile inf) {
    inf.outdir.mkdirs();
    try {
      if (verbose) System.out.println("chunk=" + inf.luachunkname + " srcfile=" + inf.srcfilename);

      // create the chunk
      FileInputStream fis = new FileInputStream(inf.infile);
      final Hashtable t =
          encoding != null
              ? LuaJC.instance.compileAll(
      ...
      for (Enumeration e = t.keys(); e.hasMoreElements(); ) {
        ...
        if (key.indexOf('/') >= 0) {
          String d =
              (destdir != null ? destdir + "/" : "") + key.substring(0, key.lastIndexOf('/'));
          new File(d).mkdirs();
        }
        String destpath = (destdir != null ? destdir + "/" : "") + key + ".class";
        if (verbose) System.out.println("  " + destpath + " (" + bytes.length + " bytes)");
      ...
      if (loadclasses) {
        ...
        for (Enumeration e = t.keys(); e.hasMoreElements(); ) {
          ...
          try {
            ...
            if (verbose) System.out.println("    loaded " + classname + " as " + o);
          } catch (Exception ex) {
            System.out.flush();
